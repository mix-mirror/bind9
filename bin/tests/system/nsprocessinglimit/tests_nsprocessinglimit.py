...
def expect_ips(expected_ips, expected_query, expected_len, ips_and_queries):
    tail = list(ips_and_queries)
    assert expected_len == len(expected_ips)
    assert expected_len == len(tail)
    for expected_ip in expected_ips:
        found = False
        for ip, query in tail:
            if ip == expected_ip:
