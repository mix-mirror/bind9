...
ans2_control() {
  control_name=$1
  shift
  $DIG -p "${PORT}" @10.53.0.2 "${control_name}.send-responses._control" TXT "$@" >dig.out.ans2_control || return 1
  grep "status: NOERROR" dig.out.ans2_control >/dev/null || return 1
}
