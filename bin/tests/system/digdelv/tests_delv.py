...
def test_cookie(delv, ns3):
    """Check that delv +cookie sends a DNS COOKIE option."""
    result = delv(f"@{ns3.ip} +cookie a a.example -d 10")
    assert "; COOKIE:" in result.err
    assert "; answer not validated" in result.out


def test_nocookie(delv, ns3):
    """Check that delv +nocookie does not send a DNS COOKIE option."""
    result = delv(f"@{ns3.ip} +nocookie +strace a a.example -d 10")
    assert "; COOKIE:" not in result.err
    assert "; answer not validated" in result.out
...
def test_ns_address_family(delv, family):
    ...
    family."""
    ipv4_packet = Re(r"sending packet from [0-9.]*#[0-9]* to")
    ipv6_packet = Re(r"sending packet from [0-9a-f:]*#[0-9]* to")
    result = delv(
...
@pytest.mark.parametrize(
    "option,expect_cookie",
    [
        param("+cookie", True),
        param("+nocookie", False),
    ],
)
@pytest.mark.usefixtures("ns1")
def test_ns_cookie(delv, option, expect_cookie):
    """Check that delv +ns +[no]cookie controls the DNS COOKIE options."""
    result = delv(
        f"-a ns1/anchor.dnskey +root +ns +qmin +hint=root.hint {option} -d 99 a a.example"
    )
    assert ("; COOKIE:" in result.out) == expect_cookie
    assert "; fully validated" in result.out
