...
check_manytypes() (
  ...
  if [ -n "$ttl" ] && ! grep -Eq "^$exname.[[:space:]]+${ttl}[[:space:]]+IN[[:space:]]+$extype" "dig.out.$i.$type.test$n"; then
    exit 1
  fi

  if [ -n "${neq_ttl}" ] && grep -Eq "^$exname.[[:space:]]+${neq_ttl}[[:space:]]+IN[[:space:]]+$extype" "dig.out.$i.$type.test$n"; then
    exit 1
  fi
...
for ntype in $(seq 64280 64289); do
...
for ntype in $(seq 64280 64534); do
  check_manytypes 1 manytypes.big "TYPE${ntype}" NOERROR manytypes.big "TYPE${ntype}" 120 || ret=1
done
# Wait at least one second
sleep 1
