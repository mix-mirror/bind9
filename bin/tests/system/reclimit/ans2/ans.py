...
class RecursionChainHandler(ResponseHandler):
    """Simulate a delegation chain for recursion depth/query limit testing.

    Chain queries (ns1.N.example.org) use a delayed response mechanism:
    when the resolver sends both A and AAAA queries for a chain name,
    the first response is held until the second query arrives (or a 0.5s
    timeout expires).  This matches the original Perl ans2 behavior.
    """
    ...
    async def _maybe_delay(self):
        ...
        If another chain query is already waiting, signal it and return
        immediately.  Otherwise, wait up to 0.5s for an accompanying
        query.  If the timeout expires, stop waiting for future queries.
        """
        if self._no_more_waiting:
            return

        if self._delayed_pending:
            # Another query is waiting -- signal it and proceed.
            self._delayed_event.set()
            return

        # First chain query -- wait for the accompanying query.
        self._delayed_pending = True
        self._delayed_event.clear()
        try:
            await asyncio.wait_for(self._delayed_event.wait(), timeout=0.5)
        except asyncio.TimeoutError:
            # No accompanying query arrived.  Stop waiting in the future.
    ...
    async def get_responses(
        ...
        if qname == "count":
            ...
            nxt = n + 1
            wait = True
            if self.limit == 0 or (not self.send_response and nxt <= self.limit):
                response.authority.append(
                    dns.rrset.from_text(
                        f"{n}.example.org.",
                        86400,
