...
def configure_cskroll2(tld: str, policy: str) -> List[Zone]:
    ...
    csk2_name = keygen(f"{newtimes} {zonename}", cwd="ns3").strip()
    settime(
        f"-g HIDDEN -k OMNIPRESENT {TactN} -r OMNIPRESENT {TactN} -z UNRETENTIVE {TactN1} -d UNRETENTIVE {TretN} -D ds {TactN1} {csk1_name}",
        cwd="ns3",
    )
    settime(
        f"-g OMNIPRESENT -k OMNIPRESENT {TactN1} -r OMNIPRESENT {TactN1} -z RUMOURED {TactN1} -d RUMOURED {TactN1} -P ds {TactN1} {csk2_name}",
...
def configure_enable_dnssec(tld: str, policy: str) -> List[Zone]:
    ...
    csk_name = keygen(f"{keytimes} {zonename}", cwd="ns3").strip()
    settime(
        f"-g OMNIPRESENT -k OMNIPRESENT {TpubN} -r OMNIPRESENT {TpubN} -z OMNIPRESENT {TpubN} -d RUMOURED {TpubN} -P ds {TsbmN} {csk_name}",
        cwd="ns3",
    )
    # Signing.
    render_and_sign_zone(zonename, [csk_name], extra_options="-z")
...
def configure_ksk_doubleksk(tld: str) -> List[Zone]:
    ...
    settime(
        f"-g HIDDEN -k OMNIPRESENT {TpubN} -r OMNIPRESENT {TpubN} -d OMNIPRESENT {TactN} {ksk1_name}",
        cwd="ns3",
    )
    settime(
        f"-g OMNIPRESENT -k RUMOURED {TpubN1} -r RUMOURED {TpubN1} -d HIDDEN {TpubN1} {ksk2_name}",
