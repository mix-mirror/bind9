...
def test_rollover_manual(servers):
    ...
    retire_key(expected, 0, config, ksknow)
    ...
    retire_key(expected, 0, config, ksknow)
    retire_key(expected, 2, config, zsknow)
    ...
    isctest.kasp.check_keyrelationships(keys, expected)
    isctest.kasp.check_keytimes(keys, expected)
    isctest.kasp.check_dnssecstatus(ns4, zone, keys, policy=policy)
    isctest.kasp.check_apex(ns4, zone, ksks, zsks)
    isctest.kasp.check_subdomain(ns4, zone, ksks, zsks)
