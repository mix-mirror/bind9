...
resolve_cb(dns_client_t *client, const dns_name_t *query_name,
	...
	ISC_LIST_FOREACH(*namelist, response_name, link) {
		ISC_LIST_FOREACH(response_name->list, rdataset, link) {
			printdata(rdataset, &response_name->name);
		}
	}
...
recvresponse(void *arg) {
	...
	MSG_SECTION_FOREACH(response, DNS_SECTION_ANSWER, name) {
		...
		ISC_LIST_FOREACH(name->list, rdataset, link) {
			 ...
			 */
			if (prev != NULL && dns_name_equal(prev, &name->name)) {
				continue;
			}
			prev = &name->name;
			...
			result = dns_view_simplefind(view, &name->name, rdataset->type,
						     0, options, false, &rds,
						     &sigs);
			if (result == ISC_R_SUCCESS) {
				printdata(&rds, &name->name);
				dns_rdataset_disassociate(&rds);
				if (dns_rdataset_isassociated(&sigs)) {
					printdata(&sigs, &name->name);
...
sendquery(void *arg) {
	...
	dns_name_clone(query_name, &mname->name);
