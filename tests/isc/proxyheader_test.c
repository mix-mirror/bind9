...
setup_test_proxy(void **state) {
	...
	*handler = isc_proxy2_handler_new(0, proxy2_handler_dummy, NULL);
	return 0;
}
...
ISC_RUN_TEST_IMPL(proxyheader_max_size_test) {
	...
	isc_proxy2_handler_init(&handler, sizeof(proxy_v2_header),
	...
	isc_proxy2_handler_init(&handler, sizeof(proxy_v2_header) - 1,
				proxy2_handler_dummy, NULL);
...
proxy2_handler_rebuild(isc_buffer_t *restrict outbuf, const void *data,
	...
	isc_proxy2_handler_init(&handler, 0, proxy2_handler_rebuild_cb, outbuf);
