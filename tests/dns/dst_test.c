...
check_cmp(const char *key1_name, dns_keytag_t key1_id, const char *key2_name,
	...
	result = dns_fixedname_fromtext(&fname1, &b1, dns_rootname, 0);
	assert_int_equal(result, ISC_R_SUCCESS);
	...
	result = dns_fixedname_fromtext(&fname2, &b2, dns_rootname, 0);
	assert_int_equal(result, ISC_R_SUCCESS);
...
ISC_RUN_TEST_IMPL(ecdsa_determinism_test) {
	...
	result = dns_fixedname_fromtext(&fname, &keybuf, dns_rootname, 0);
	assert_int_equal(result, ISC_R_SUCCESS);
