...
lookupdb(dns_delegdb_t *db, const char *namestr, isc_stdtime_t now,
	...
	result = dns_deleg_lookup(db, name, now, options, zonecut, delegsetp);
...
ISC_RUN_TEST_IMPL(dns_deleg_noexacttests) {
	...
	for (size_t i = 0; i < ARRAY_SIZE(zonecuts); i++) {
		char nsname[DNS_NAME_MAXWIRE] = { 0 };

		sprintf(nsname, "ns.%s", zonecuts[i].name);
		dns_deleg_allocset(db, &delegset);
		dns_deleg_allocdeleg(delegset, &deleg);
		addnamedeleg(nsname, delegset, deleg, dns_deleg_addns);
	...
	for (size_t i = 0; i < ARRAY_SIZE(zonecuts); i++) {
		...
		assert_int_equal(result, zonecuts[i].expected[0] == 0
						 ? ISC_R_NOTFOUND
						 : ISC_R_SUCCESS);
		if (delegset != NULL) {
			dns_delegset_detach(&delegset);
		...
		assert_int_equal(result, zonecuts[i].noexactexpected[0] == 0
						 ? ISC_R_NOTFOUND
						 : ISC_R_SUCCESS);
		if (delegset != NULL) {
			dns_delegset_detach(&delegset);
