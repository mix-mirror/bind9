...
ISC_RUN_TEST_IMPL(rdatavec_subtract_assertion_failure) {
	isc_mem_t *mctx = isc_g_mctx;
	UNUSED(state);
	dns_vecheader_t *original_header = NULL, *subtract_header = NULL, *result_header = NULL;
	dns_rdataset_t original_rdataset, subtract_rdataset;
	dns_rdatalist_t *original_rdatalist = NULL, *subtract_rdatalist = NULL;
	dns_rdata_t *original_rdata1 = NULL, *original_rdata2 = NULL, *subtract_rdata = NULL;
	unsigned char *original_data1 = NULL, *original_data2 = NULL, *subtract_data = NULL;
	...
	CHECK(dns_rdatavec_fromrdataset(&original_rdataset, mctx, &original_region, 0));
	original_header = (dns_vecheader_t *)original_region.base;

	CHECK(dns_rdatavec_fromrdataset(&subtract_rdataset, mctx, &subtract_region, 0));
	 ...
	 * Since we're subtracting 1 record from 2, it should create a new header
	 * and hit the problematic code path at rdatavec.c:759
	 */
...
cleanup:
	...
	if (original_rdatalist != NULL) {
		isc_mem_put(mctx, original_rdatalist, sizeof(*original_rdatalist));
	}
...
ISC_TEST_ENTRY_CUSTOM(rdatavec_subtract_assertion_failure, setup_mctx, teardown_mctx)
