...
static isc_result_t
mock_dump(dst_key_t *key, isc_mem_t *key_mctx, char **buffer, int *length) {
	UNUSED(key);
	...
	*buffer = isc_mem_get(key_mctx, *length);
...
add_key(dns_tsigkeyring_t *ring, const char *namestr, bool generated,
	...
	result = dns_tsigkey_createfromkey(name, DST_ALG_HMACSHA256, tmp->key,
					   generated, false, creator,
					   TEST_INCEPTION, expire, mctx, &tkey);
	assert_int_equal(result, ISC_R_SUCCESS);
...
ISC_TEST_ENTRY_CUSTOM(tsig_delete, setup_test, teardown_test)
ISC_TEST_ENTRY_CUSTOM(tsig_tcp, setup_test, teardown_test)
ISC_TEST_ENTRY_CUSTOM(tsig_dumpanddetach_shared, setup_test, teardown_test)
ISC_TEST_ENTRY_CUSTOM(tsig_dumpanddetach_nothing, setup_test, teardown_test)
ISC_TEST_ENTRY_CUSTOM(tsig_dumpanddetach_key, setup_test, teardown_test)
ISC_TEST_ENTRY_CUSTOM(tsig_dumpanddetach_skips_undumpable, setup_test,
		      teardown_test)
ISC_TEST_ENTRY_CUSTOM(tsig_dumpanddetach_unwritable, setup_test, teardown_test)
ISC_TEST_ENTRY_CUSTOM(tsig_dumpanddetach_concurrent, setup_test, teardown_test)
ISC_TEST_LIST_END
