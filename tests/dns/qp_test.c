 ...
 */
static void
maybe_set_name(dns_qpreader_t *qp, void *pval, uint32_t ival, dns_name_t *name,
...
qpiter_next_with_name(dns_qpiter_t *qpi, dns_name_t *name, void **pval_r,
	...
	if (result == ISC_R_SUCCESS) {
		maybe_set_name(qpi->qp, pval, ival, name, pval_r, ival_r);
	}
	return result;
}
...
check_partialmatch(dns_qp_t *qp, struct check_partialmatch check[],
	...
	for (int i = 0; check[i].query != NULL; i++) {
		...
		if ((result == ISC_R_SUCCESS || result == DNS_R_PARTIALMATCH) && pval != NULL) {
			maybe_set_name((dns_qpreader_t *)qp, pval, space, foundname, NULL, NULL);
		}
...
check_qpchainiter(dns_qp_t *qp, struct check_qpchain check[],
	...
	for (int i = 0; check[i].query != NULL; i++) {
		...
		for (unsigned int j = 0; j < check[i].length; j++) {
			...
			maybe_set_name(chain.qp, pval, ival, found, NULL, NULL);
#if 0
