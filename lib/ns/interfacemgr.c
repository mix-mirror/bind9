	...
	case ISC_R_SUCCESS:
		...
		if (rtm->rtm_version != RTM_VERSION) {
			isc_log_write(
				...
				NS_LOGMODULE_INTERFACEMGR, ISC_LOG_ERROR,
...
ns_interfacemgr_create(isc_mem_t *mctx, ns_server_t *sctx,
	...
	*mgrp = mgr;
	...
	isc_timer_create(isc_loop_main(), interface_timer_tick, mgr,
			 &mgr->route_timer);
...
setup_locals(isc_interface_t *interface, dns_acl_t *localhost,
	...
	dns_iptable_addprefix(localhost->iptable, netaddr, prefixlen,
			      RADIX_ALLOW);
	...
	dns_iptable_addprefix(localnets->iptable, netaddr, prefixlen,
			      RADIX_ALLOW);
