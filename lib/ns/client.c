...
#define USEKEEPALIVE(x) (((x)->inner.attributes & NS_CLIENTATTR_USEKEEPALIVE) != 0)
...
ns_client_killoldestquery(ns_client_t *client) {
	...
	if (oldest != NULL) {
		ISC_LIST_UNLINK(client->manager->recursing, oldest, inner.rlink);
...
ns_client_endrequest(ns_client_t *client) {
	...
	if (client->inner.opt != NULL) {
		...
		dns_message_puttemprdataset(client->message, &client->inner.opt);
	}
...
ns_client_send(ns_client_t *client) {
	...
	if (client->inner.view != NULL) {
		...
		} else if (client->inner.view->preferred_glue == dns_rdatatype_aaaa) {
...
ns_client_error(ns_client_t *client, isc_result_t result) {
	...
	if (client->inner.view != NULL && client->inner.view->rrl != NULL) {
		...
		rrl_result = dns_rrl(client->inner.view, NULL, &client->inner.peeraddr,
				     TCP_CLIENT(client), dns_rdataclass_in,
				     dns_rdatatype_none, NULL, result,
				     client->inner.now, wouldlog, log_buf,
				     sizeof(log_buf));
	...
	if (rcode == dns_rcode_formerr) {
		...
		client->inner.formerrcache.id = message->id;
		   ...
		   client->inner.view != NULL && client->inner.view->fail_ttl != 0 &&
	...
	if (result == ISC_R_SUCCESS) {
		...
		while (isc_buffer_remaininglength(&optbuf) >= 4) {
			...
			case DNS_OPT_NSID:
				...
				client->inner.attributes |= NS_CLIENTATTR_WANTNSID;
				isc_buffer_forward(&optbuf, optlen);
				break;
			case DNS_OPT_COOKIE:
			...
			case DNS_OPT_TCP_KEEPALIVE:
				...
				isc_nmhandle_keepalive(client->inner.handle, true);
				isc_buffer_forward(&optbuf, optlen);
				break;
			case DNS_OPT_PAD:
				client->inner.attributes |= NS_CLIENTATTR_WANTPAD;
				ns_stats_increment(
					client->manager->sctx->nsstats,
...
ns_client_request(isc_nmhandle_t *handle, isc_result_t eresult,
	...
	isc_netaddr_fromsockaddr(&client->inner.destaddr, &client->inner.destsockaddr);
...
ns_client_request_continue(void *arg) {
	...
	ns_client_log(client, NS_LOGCATEGORY_CLIENT, NS_LOGMODULE_CLIENT,
		      ISC_LOG_DEBUG(5), "using view '%s'", client->inner.view->name);
	...
	if (result == ISC_R_SUCCESS) {
		...
		dns_name_format(&client->inner.signername, namebuf, sizeof(namebuf));
		ns_client_log(client, DNS_LOGCATEGORY_SECURITY,
			      NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG(3),
	...
	if (client->inner.udpsize > 512) {
		...
		isc_netaddr_fromsockaddr(&netaddr, &client->inner.peeraddr);
		(void)dns_peerlist_peerbyaddr(client->inner.view->peers, &netaddr,
					      &peer);
		if (peer != NULL) {
	...
	case dns_opcode_update:
		...
		ns_update_start(client, client->inner.handle, client->inner.sigresult);
		break;
	case dns_opcode_notify:
		CTRACE("notify");
		ns_client_settimeout(client, 60);
...
ns__client_setup(ns_client_t *client, ns_clientmgr_t *mgr, bool new) {
	...
	if (new) {
		...
		*client = (ns_client_t){ .magic = 0 };
		...
		/*
		 * EDE handling allocates. We need to free out the child
		 * pointers before zero-ing out the struct.
		 */
...
ns_client_checkaclsilent(ns_client_t *client, isc_netaddr_t *netaddr,
	...
	result = dns_acl_match_port_transport(
		...
		isc_nm_has_encryption(client->inner.handle), client->inner.signer, acl, env,
		&match, NULL);
...
ns_client_logv(ns_client_t *client, isc_logcategory_t category,
	...
	if (client->inner.signer != NULL) {
		dns_name_format(client->inner.signer, signerbuf, sizeof(signerbuf));
		sep1 = "/key ";
		signer = signerbuf;
	}
	...
	if (client->inner.view != NULL && strcmp(client->inner.view->name, "_bind") != 0 &&
