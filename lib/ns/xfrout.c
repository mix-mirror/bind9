...
ns_xfr_start(ns_client_t *client, dns_rdatatype_t reqtype) {
	...
	if (result != ISC_R_SUCCESS || dns_zone_gettype(zone) == dns_zone_dlz) {
		...
		if (!ISC_LIST_EMPTY(client->inner.view->dlz_searched)) {
			result = dns_dlzallowzonexfr(client->inner.view,
						     question_name,
...
have_stream:
	...
	if (zone != NULL) {
		...
		if ((client->inner.attributes & NS_CLIENTATTR_WANTEXPIRE) != 0 &&
		     ...
		     dns_zone_gettype(mayberaw) == dns_zone_mirror))
		{
			...
			if (secs >= client->inner.now && result == ISC_R_SUCCESS) {
				client->inner.attributes |= NS_CLIENTATTR_HAVEEXPIRE;
...
xfrout_ctx_create(isc_mem_t *mctx, ns_client_t *client, unsigned int id,
	...
	*xfr = (xfrout_ctx_t){
	...
	isc_nm_timer_create(xfr->client->inner.handle, xfrout_client_timeout, xfr,
			    &xfr->maxtime_timer);

	isc_nm_timer_create(xfr->client->inner.handle, xfrout_delayed_timeout, xfr,
...
xfrout_send(xfrout_ctx_t *xfr) {
	const bool is_tcp = ((xfr->client->inner.attributes & NS_CLIENTATTR_TCP) !=
			     0);
	...
	if (is_tcp) {
		...
		if (xfr->idletime > 0) {
			isc_nmhandle_setwritetimeout(xfr->client->inner.sendhandle,
						     xfr->idletime);
		}
		isc_nm_send(xfr->client->inner.sendhandle, &used, xfrout_senddone,
			    xfr);
		xfr->sends++;
...
sendstream(xfrout_ctx_t *xfr) {
	...
	if (!is_tcp) {
		 ...
		 */
		if ((xfr->client->inner.attributes & NS_CLIENTATTR_WANTOPT) != 0) {
			 ...
			 */
			xfr->client->inner.attributes &= ~NS_CLIENTATTR_WANTNSID;
			xfr->client->inner.attributes &= ~NS_CLIENTATTR_HAVEEXPIRE;
		}
