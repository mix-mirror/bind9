...
log_response(ns_client_t *client, dns_rcode_t rcode) {
	...
	ns_client_log_flags(client, client->message->flags, client->inner.extflags,
...
query_checkcacheaccess(ns_client_t *client, const dns_name_t *name,
	...
	if ((client->query.attributes & NS_QUERYATTR_CACHEACLOKVALID) == 0) {
		...
		result = ns_client_checkaclsilent(client, NULL,
						  client->inner.view->cacheacl, true);
...
query_getdb(ns_client_t *client, dns_name_t *name, dns_rdatatype_t qtype,
	    ...
	    !ISC_LIST_EMPTY(client->inner.view->dlz_searched))
	{
		...
		tdbp = NULL;
		tresult = dns_view_searchdlz(client->inner.view, name, zonelabels,
					     &cm, &ci, &tdbp);
...
query_additional_cb(void *arg, const dns_name_t *name, dns_rdatatype_t qtype,
	...
	result = dns_db_findext(db, name, version, type,
				...
				client->inner.now, &node, fname, &cm, &ci, rdataset,
				sigrdataset);
...
found:
	...
	aaaa_lookup:
		...
		result = dns_db_findrdataset(db, node, version,
					     dns_rdatatype_aaaa, 0, client->inner.now,
					     rdataset, sigrdataset);
		if (result == DNS_R_NCACHENXDOMAIN) {
			goto addname;
		} else if (result == DNS_R_NCACHENXRRSET) {
...
addname:
	...
	if (trdataset != NULL && dns_rdatatype_followadditional(type)) {
		if (client->inner.additionaldepth++ < client->inner.view->max_restarts) {
...
mark_secure(ns_client_t *client, dns_db_t *db, dns_name_t *name,
	...
	(void)dns_db_addrdataset(db, node, NULL, client->inner.now, rdataset, 0,
				 NULL);
	(void)dns_db_addrdataset(db, node, NULL, client->inner.now, sigrdataset, 0,
				 NULL);
	dns_db_detachnode(db, &node);
}
...
stale_refresh_aftermath(ns_client_t *client, isc_result_t result) {
	...
	default:
		...
		(void)dns_db_findext(db, qctx.client->query.qname, NULL,
				     ...
				     qctx.client->inner.now, &qctx.node, qctx.fname,
...
rpz_rrset_find(ns_client_t *client, dns_name_t *name, dns_rdatatype_t type,
	...
	dns_clientinfomethods_init(&cm, ns_client_sourceip);
	dns_clientinfo_init(&ci, client, NULL);
	result = dns_db_findext(*dbp, name, version, type, options, client->inner.now,
...
rpz_find_p(ns_client_t *client, dns_name_t *self_name, dns_rdatatype_t qtype,
	...
	*versionp = NULL;
	...
	result = dns_db_findext(*dbp, p_name, *versionp, dns_rdatatype_any, 0,
				client->inner.now, nodep, found, &cm, &ci, *rdatasetp,
				NULL);
	/*
...
again:
	...
	result = dns_db_findext(db, dns_fixedname_name(&fixed), version,
				dns_rdatatype_nsec3, dboptions, client->inner.now,
...
redirect(ns_client_t *client, dns_name_t *name, dns_rdataset_t *rdataset,
	...
	result = ns_client_checkaclsilent(
		client, NULL, dns_zone_getqueryacl(client->inner.view->redirect),
		true);
	if (result != ISC_R_SUCCESS) {
		return ISC_R_NOTFOUND;
	}
	...
	result = dns_db_findext(db, client->query.qname, dbversion->version,
				qtype, DNS_DBFIND_NOZONECUT, client->inner.now, &node,
...
redirect2(ns_client_t *client, dns_name_t *name, dns_rdataset_t *rdataset,
	...
	if (labels > 1U) {
		...
		result = dns_name_concatenate(
			&prefix, client->inner.view->redirectzone, redirectname);
		if (result != ISC_R_SUCCESS) {
			return ISC_R_NOTFOUND;
		}
	} else {
	...
	result = dns_db_findext(db, redirectname, version, qtype, 0,
				client->inner.now, &node, found, &cm, &ci, &trdataset,
				NULL);
	if (result == DNS_R_NXRRSET || result == DNS_R_NCACHENXRRSET) {
		if (dns_rdataset_isassociated(rdataset)) {
			dns_rdataset_disassociate(rdataset);
	 ...
	 */
	dns_name_split(found, dns_name_countlabels(client->inner.view->redirectzone),
		       found, NULL);
	/*
...
qctx_setrad(query_ctx_t *qctx) {
	...
	if ((client->inner.attributes & NS_CLIENTATTR_WANTRC) != 0) {
		...
		{
			dns_name_dup(rad, client->manager->mctx, &client->inner.rad);
		}
	}
}
...
fetch_callback(void *arg) {
	 ...
	 */
	if (client->inner.view->cachedb != NULL && client->inner.view->recursion) {
		client->query.attributes |= NS_QUERYATTR_RECURSIONOK;
	}
	client->query.dboptions &= ~DNS_DBFIND_STALETIMEOUT;
...
release_recursionquota(ns_client_t *client) {
	...
	if (ISC_LINK_LINKED(client, inner.rlink)) {
		ISC_LIST_UNLINK(client->manager->recursing, client, inner.rlink);
	}
...
ns_query_recurse(ns_client_t *client, dns_rdatatype_t qtype, dns_name_t *qname,
	...
	isc_nmhandle_attach(client->inner.handle, &HANDLE_RECTYPE_NORMAL(client));
	result = dns_resolver_createfetch(
		client->inner.view->resolver, qname, qtype, qdomain, nameservers,
...
query_checkrrl(query_ctx_t *qctx, isc_result_t result) {
	    ...
	    (qctx->client->query.attributes & NS_QUERYATTR_RRL_CHECKED) == 0)
	{
		...
		rrl_result = dns_rrl(
			...
			qctx->qtype, constname, resp_result, qctx->client->inner.now,
...
query_getexpire(query_ctx_t *qctx) {
	    ...
	    dns_zone_gettype(mayberaw) == dns_zone_mirror)
	{
		...
		if (secs >= qctx->client->inner.now && qctx->result == ISC_R_SUCCESS)
		{
			qctx->client->inner.attributes |= NS_CLIENTATTR_HAVEEXPIRE;
			qctx->client->inner.expire = secs - qctx->client->inner.now;
		}
...
query_getzoneversion(query_ctx_t *qctx) {
	...
	if (qctx->zone == NULL || !qctx->is_zone ||
	    qctx->client->query.restarts != 0 ||
	    (qctx->client->inner.attributes & NS_CLIENTATTR_WANTZONEVERSION) == 0 ||
	    (qctx->client->inner.attributes & NS_CLIENTATTR_HAVEZONEVERSION) != 0)
	{
		return;
	}
...
query_dns64(query_ctx_t *qctx) {
	...
	result = dns_dns64_apply(
		...
		client->message, client->manager->aclenv, &client->inner.peeraddr,
		client->inner.signer, flags, qctx->rdataset, &dns64_rdataset);
	if (result != ISC_R_SUCCESS) {
		goto cleanup;
	}
...
query_notfound(query_ctx_t *qctx) {
	...
	if (qctx->view->hints != NULL) {
		...
		result = dns_db_findext(qctx->db, dns_rootname, NULL,
					dns_rdatatype_ns, 0, qctx->client->inner.now,
...
query_addds(query_ctx_t *qctx) {
	...
	result = dns_db_findrdataset(qctx->db, qctx->node, qctx->version,
				     dns_rdatatype_ds, 0, client->inner.now, rdataset,
				     sigrdataset);
	/*
...
query_coveringnsec(query_ctx_t *qctx) {
	...
	result = dns_db_findext(db, wild, qctx->version, qctx->type,
				dboptions | DNS_DBFIND_COVERINGNSEC,
				qctx->client->inner.now, &node, nowild, &cm, &ci,
				&rdataset, &sigrdataset);
	...
	result = dns_db_findext(db, signer, qctx->version, dns_rdatatype_soa,
				dboptions, qctx->client->inner.now, &node, fname, &cm,
				&ci, soardataset, sigsoardataset);
...
query_addsoa(query_ctx_t *qctx, unsigned int override_ttl,
	...
	if (result == ISC_R_SUCCESS) {
		result = dns_db_findrdataset(qctx->db, node, qctx->version,
					     dns_rdatatype_soa, 0, client->inner.now,
					     rdataset, sigrdataset);
	} else {
...
query_addns(query_ctx_t *qctx) {
	...
	if (result == ISC_R_SUCCESS) {
		result = dns_db_findrdataset(qctx->db, node, qctx->version,
					     dns_rdatatype_ns, 0, client->inner.now,
					     rdataset, sigrdataset);
	} else {
...
db_find:
	...
	if (is_zone) {
		result = dns_db_findext(
			...
			client->query.dboptions, client->inner.now, &node, fname, &cm,
		...
		result = dns_db_findzonecut(
			...
			client->inner.now, &node, fname, NULL, rdataset, sigrdataset);
		if (result == ISC_R_SUCCESS) {
			if (zfname != NULL &&
...
ns_query_start(ns_client_t *client, isc_nmhandle_t *handle) {
	 ...
	 */
	if (client->inner.ednsversion >= 0 && client->inner.udpsize <= 512U && !TCP(client))
	{
		client->query.attributes |= (NS_QUERYATTR_NOAUTHORITY |
					     NS_QUERYATTR_NOADDITIONAL);
	}
	 ...
	 */
	isc_counter_create(client->manager->mctx, client->inner.view->max_queries,
			   &client->query.qc);
