...
query_delegation_recurse(query_ctx_t *qctx) {
	...
	if (dns_rdatatype_atparent(qctx->type)) {
		...
		// isc_result_t tresult;
		//
		// tresult = dns_view_bestzonecut(qctx->view, qname, fname,
		// NULL, 			       qctx->client->inner.now,
		// 0, true, 			       true, &delegset); if
		// (tresult != ISC_R_SUCCESS) { 	INSIST(delegset ==
		// NULL);
		// dns_delegset_fromrdataset(qctx->client->manager->mctx,
