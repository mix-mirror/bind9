...
client_trace(ns_client_t *client, int level, const char *message) {
	if (client != NULL && client->query.qname != NULL) {
		if (isc_log_wouldlog(level)) {
			...
			dns_name_format(&client->query.qname->name, qbuf,
					sizeof(qbuf));
...
log_response(ns_client_t *client, dns_rcode_t rcode) {
	...
	dns_name_format(&client->query.origqname->name, namebuf, sizeof(namebuf));
...
rpz_log_rewrite(ns_client_t *client, bool disabled, dns_rpz_policy_t policy,
	...
	dns_name_format(&client->query.qname->name, qname_buf, sizeof(qname_buf));
...
rpz_log_fail_helper(ns_client_t *client, int level, dns_name_t *p_name,
	...
	dns_name_format(&client->query.qname->name, qnamebuf, sizeof(qnamebuf));
...
rpz_getdb(ns_client_t *client, dns_name_t *p_name, dns_rpz_type_t rpz_type,
	...
	if (result == ISC_R_SUCCESS) {
		...
		{
			dns_name_format(&client->query.qname->name, qnamebuf,
					sizeof(qnamebuf));
			dns_name_format(p_name, p_namebuf, sizeof(p_namebuf));
...
query_isduplicate(ns_client_t *client, dns_name_t *name, dns_rdatatype_t type,
		  dns_name_with_links_t **mnamep) {
	dns_section_t section;
	dns_name_with_links_t *mname_wl = NULL;
	isc_result_t result;
	...
	SET_IF_NOT_NULL(mnamep, mname_wl);
...
query_additional_cb(void *arg, const dns_name_t *name, dns_rdatatype_t qtype,
	...
	dns_dbnode_t *node = NULL;
	dns_db_t *db = NULL;
	dns_name_with_links_t *fname = NULL, *mname = NULL;
	 ...
	 */
	result = query_additionalauth(qctx, name, type, &db, &node, &fname->name,
				      rdataset, sigrdataset);
	if (result == ISC_R_SUCCESS) {
		goto found;
	}
	...
	result = dns_db_findext(db, name, version, type,
				...
				client->inner.now, &node, &fname->name, &cm, &ci,
				rdataset, sigrdataset);
...
found:
	 ...
	 * section.
	 */
	dns_name_with_links_t *temp_mname = NULL;
	if (dns_rdataset_isassociated(rdataset) &&
	    !query_isduplicate(client, &fname->name, type, &temp_mname))
	{
		if (temp_mname != NULL) {
			INSIST(temp_mname != fname);
	...
	if (qtype == dns_rdatatype_a) {
		...
		if (query_isduplicate(client, &fname->name, dns_rdatatype_a, NULL)) {
			goto aaaa_lookup;
		}
		...
		if (result == DNS_R_NCACHENXDOMAIN) {
			     ...
			     DNS_TRUST_GLUE(rdataset->trust)))
			{
				...
				invalid = !validate(client, db, &fname->name, rdataset,
						    sigrdataset);
			}
			if (invalid && DNS_TRUST_PENDING(rdataset->trust)) {
				dns_rdataset_cleanup(rdataset);
				dns_rdataset_cleanup(sigrdataset);
			} else if (!query_isduplicate(client, &fname->name,
	...
	aaaa_lookup:
		if (query_isduplicate(client, &fname->name, dns_rdatatype_aaaa, NULL))
		{
			goto addname;
		}
	...
	if (trdataset != NULL && dns_rdatatype_followadditional(type)) {
		    ...
		    client->inner.view->max_restarts)
		{
			eresult = dns_rdataset_additionaldata(
				trdataset, &fname->name, query_additional_cb, qctx,
 ...
 */
static void
query_addtoname(dns_name_with_links_t *name, dns_rdataset_t *rdataset) {
...
static void
query_addrrset(query_ctx_t *qctx, dns_name_with_links_t **namep,
	       dns_rdataset_t **rdatasetp, dns_rdataset_t **sigrdatasetp,
	       isc_buffer_t *dbuf, dns_section_t section) {
	isc_result_t result;
	ns_client_t *client = qctx->client;
	dns_name_with_links_t *name = *namep;
	dns_name_with_links_t *mname = NULL;
	 ...
	 */
	result = dns_message_findname(client->message, section, &name->name,
	...
	query_setorder(qctx, &mname->name, rdataset);
	query_additional(qctx, &mname->name, rdataset);
...
static void
fixfname(ns_client_t *client, dns_name_with_links_t **fname, isc_buffer_t **dbuf,
	 isc_buffer_t *nbuf) {
	if (*fname == NULL) {
		*dbuf = ns_client_getnamebuf(client);
...
stale_refresh_aftermath(ns_client_t *client, isc_result_t result) {
	...
	default:
		...
		(void)dns_db_findext(db, &qctx.client->query.qname->name, NULL,
				     qctx.client->query.qtype, dboptions,
				     qctx.client->inner.now, &qctx.node,
				     &qctx.fname->name, &cm, &ci, qctx.rdataset,
				     qctx.sigrdataset);
		if (qctx.node != NULL) {
...
query_stale_refresh_ncache(ns_client_t *client) {
	...
	if (client->query.origqname != NULL) {
		qname = &client->query.origqname->name;
	} else {
		qname = &client->query.qname->name;
	}
...
rpz_rewrite(ns_client_t *client, dns_rdatatype_t qtype, isc_result_t qresult,
	    ...
	    (DNS_RPZ_DONE_CLIENT_IP | DNS_RPZ_DONE_QNAME))
	{
		...
		if ((st->state & DNS_RPZ_DONE_QNAME) == 0) {
			CHECK(rpz_rewrite_name(client, &client->query.qname->name,
			 ...
			 */
			st->r.label = dns_name_countlabels(&client->query.qname->name);
	...
	{
		CHECK(rpz_rewrite_ip_rrsets(client, &client->query.qname->name, qtype,
	...
	dns_name_clone(&client->query.qname->name, dns_fixedname_name(&nsnamef));
	options = client->query.dboptions | DNS_DBFIND_GLUEOK;
	while (st->r.label > st->popt.min_ns_labels) {
		 ...
		 */
		if (st->r.label == dns_name_countlabels(&client->query.qname->name)) {
			nsname = &client->query.qname->name;
		} else {
			nsname = dns_fixedname_name(&nsnamef);
			dns_name_split(&client->query.qname->name, st->r.label, NULL,
				       nsname);
		}
		if (st->r.ns_rdataset == NULL ||
...
redirect(ns_client_t *client, dns_name_t *name, dns_rdataset_t *rdataset,
	 ...
	 */
	result = dns_db_findext(db, &client->query.qname->name, dbversion->version,
...
redirect2(ns_client_t *client, dns_name_t *name, dns_rdataset_t *rdataset,
	...
	labels = dns_name_countlabels(&client->query.qname->name);
	if (labels > 1U) {
		dns_name_t prefix;

		dns_name_init(&prefix);
		dns_name_getlabelsequence(&client->query.qname->name, 0, labels - 1,
					  &prefix);
...
query_trace(query_ctx_t *qctx) {
	...
	if (qctx->client->query.origqname != NULL) {
		dns_name_format(&qctx->client->query.origqname->name, qbuf,
				sizeof(qbuf));
	} else {
...
root_key_sentinel_detect(query_ctx_t *qctx) {
	const char *ndata = (const char *)qctx->client->query.qname->name.ndata;

	if (qctx->client->query.qname->name.length > 30 && ndata[0] == 29 &&
	...
	} else if (qctx->client->query.qname->name.length > 31 && ndata[0] == 30 &&
...
qctx_reportquery(query_ctx_t *qctx) {
	...
	if (client->query.qtype != dns_rdatatype_txt ||
	    !dns_name_israd(&client->query.qname->name,
...
ns__query_start(query_ctx_t *qctx) {
	...
	if (qctx->view->checknames &&
	    !dns_rdata_checkowner(&qctx->client->query.qname->name,
				  ...
				  false))
	{
		...
		dns_name_format(&qctx->client->query.qname->name, namebuf,
				sizeof(namebuf));
	...
	if (dns_rdatatype_atparent(qctx->qtype) &&
	    !dns_name_equal(&qctx->client->query.qname->name, dns_rootname))
	{
		/*
	...
	result = query_getdb(qctx->client, &qctx->client->query.qname->name,
	    ...
	    qctx->options.noexact)
	{
		...
		tresult = query_getzonedb(
			qctx->client, &qctx->client->query.qname->name, qctx->qtype,
...
query_lookup(query_ctx_t *qctx) {
	...
	if (qctx->dns64 && qctx->rpz) {
		...
		rpzqname = &qctx->client->query.qname->name;
	}
	 ...
	 */
	if (qctx->dns64 && qctx->rpz) {
		dns_name_copy(&qctx->client->query.qname->name, &qctx->fname->name);
		dns_rdataset_cleanup(qctx->sigrdataset);
	}
...
query_resume(query_ctx_t *qctx) {
	...
	dns_name_copy(tname, &qctx->fname->name);
...
ns__query_sfcache(query_ctx_t *qctx) {
...
#endif /* ifdef ENABLE_AFL */
	{
		failcache = dns_badcache_find(
			qctx->view->failcache, &qctx->client->query.qname->name,
			qctx->qtype, &flags,
...
query_trace_rrldrop(query_ctx_t *qctx,
	...
	dns_name_format(&qctx->client->query.qname->name, qnamebuf, sizeof(qnamebuf));
	dns_name_format(&qctx->fname->name, fnamebuf, sizeof(fnamebuf));
...
query_checkrrl(query_ctx_t *qctx, isc_result_t result) {
	...
	ns_client_log(
		...
		qctx->fname != NULL ? dns_name_isabsolute(&qctx->fname->name) : 0,
	...
	if (qctx->view->rrl != NULL && !HAVECOOKIE(qctx->client) &&
	    ((qctx->fname != NULL && dns_name_isabsolute(&qctx->fname->name)) ||
...
query_checkrpz(query_ctx_t *qctx, isc_result_t result) {
	...
	case DNS_R_DELEGATION:
		...
		dns_name_copy(&qctx->fname->name, qctx->rpz_st->fname);
...
query_rpzcname(query_ctx_t *qctx, dns_name_t *cname) {
	...
	if (labels > 2 && dns_name_iswildcard(cname)) {
		...
		dns_name_split(&client->query.qname->name, 1,
		...
		dns_name_copy(cname, &qctx->fname->name);
	}
	...
	rpz_log_rewrite(client, false, qctx->rpz_st->m.policy,
			...
			qctx->rpz_st->p_name, &qctx->fname->name,
			qctx->rpz_st->m.rpz->num);
...
query_usestale(query_ctx_t *qctx, isc_result_t result) {
	...
	if (dns_view_staleanswerenabled(qctx->client->inner.view)) {
		if (query_getdb(qctx->client, &qctx->client->query.qname->name,
...
query_gotanswer(query_ctx_t *qctx, isc_result_t result) {
	...
	if (!dns_name_equal(&qctx->client->query.qname->name, dns_rootname)) {
...
query_addnoqnameproof(query_ctx_t *qctx) {
	ns_client_t *client = qctx->client;
	isc_buffer_t *dbuf, b;
	dns_name_with_links_t *fname = NULL;
	...
	result = dns_rdataset_getnoqname(qctx->noqname, &fname->name, neg, negsig);
	RUNTIME_CHECK(result == ISC_R_SUCCESS);
	...
	result = dns_rdataset_getclosest(qctx->noqname, &fname->name, neg, negsig);
	RUNTIME_CHECK(result == ISC_R_SUCCESS);
...
query_respond_any(query_ctx_t *qctx) {
	...
	DNS_RDATASETITER_FOREACH(rdsiter) {
			   ...
			   qctx->rdataset->type != 0)
		{
			...
			if (!qctx->is_zone && RECURSIONOK(qctx->client)) {
				dns_name_t *name;
				name = (qctx->fname != NULL) ? &qctx->fname->name
							     : &qctx->tname->name;
	...
	if (qctx->fname != NULL) {
		dns_name_with_links_t *tmp_wl = qctx->fname;
		dns_message_puttempname(qctx->client->message, &tmp_wl);
		qctx->fname = NULL;
	}
...
query_addanswer(query_ctx_t *qctx) {
	...
	if (qctx->dns64) {
		...
		if (!qctx->is_zone && RECURSIONOK(qctx->client)) {
			query_prefetch(qctx->client, &qctx->fname->name,
				       qctx->rdataset);
		}
...
query_respond(query_ctx_t *qctx) {
	...
	if (qctx->is_zone && qctx->qtype == dns_rdatatype_ns) {
		 ...
		 */
		if (dns_name_equal(&qctx->client->query.qname->name,
...
query_dns64(query_ctx_t *qctx) {
	isc_result_t result;
	ns_client_t *client = qctx->client;
	dns_name_with_links_t *name = NULL, *mname = NULL;
	...
	result = dns_message_findname(
		client->message, section, &name->name, dns_rdatatype_aaaa,
	...
	dns_rdataset_setownercase(dns64_rdataset, &mname->name);
	...
	query_setorder(qctx, &mname->name, dns64_rdataset);
...
query_filter64(query_ctx_t *qctx) {
	...
	dns_rdataset_setownercase(myrdataset, &name->name);
	client->query.attributes |= NS_QUERYATTR_NOADDITIONAL;
	if (mname == name) {
		if (qctx->dbuf != NULL) {
			ns_client_keepname(client, name, qctx->dbuf);
	...
	query_setorder(qctx, &mname->name, myrdataset);
...
query_notfound(query_ctx_t *qctx) {
	...
	if (qctx->view->hints != NULL) {
		...
		result = dns_db_findext(
			...
			qctx->client->inner.now, &qctx->node, &qctx->fname->name, &cm,
...
query_prepare_delegation_response(query_ctx_t *qctx) {
	...
	dns_name_copy(&qctx->fname->name, dns_fixedname_name(&qctx->dsname));
...
query_zone_delegation(query_ctx_t *qctx) {
	    ...
	    (qctx->options.noexact && qctx->qtype == dns_rdatatype_ds))
	{
		...
		result = query_getzonedb(qctx->client,
					 &qctx->client->query.qname->name, qctx->qtype,
					 options, &tzone, &tdb, &tversion);
		if (result != ISC_R_SUCCESS) {
			if (tdb != NULL) {
				dns_db_detach(&tdb);
...
query_delegation(query_ctx_t *qctx) {
	...
	if (qctx->zfname != NULL &&
	    (!dns_name_issubdomain(&qctx->fname->name, &qctx->zfname->name) ||
	     (qctx->is_staticstub_zone &&
	      dns_name_equal(&qctx->fname->name, &qctx->zfname->name))))
	{
		/*
...
query_delegation_recurse(query_ctx_t *qctx) {
	isc_result_t result = ISC_R_UNSET;
	dns_name_t *qname = &qctx->client->query.qname->name;
...
query_addds(query_ctx_t *qctx) {
	...
	MSG_SECTION_FOREACH(client->message, DNS_SECTION_AUTHORITY, rname) {
		...
		if (result == ISC_R_SUCCESS) {
			 ...
			 * delegation.
			 */
			query_addrrset(qctx, &rname, &rdataset,
				       &sigrdataset, NULL,
				       DNS_SECTION_AUTHORITY);
			break;
		}
...
addnsec3:
	...
	if (!dns_name_equal(name, dns_fixedname_name(&fixed))) {
		...
		query_findclosestnsec3(dns_fixedname_name(&fixed), qctx->db,
				       ...
				       sigrdataset, &fname->name, false, NULL);
		if (!dns_rdataset_isassociated(rdataset)) {
			goto cleanup;
		}
		query_addrrset(qctx, &fname, &rdataset, &sigrdataset, dbuf,
...
query_sign_nodata(query_ctx_t *qctx) {
	...
	if (!dns_rdataset_isassociated(qctx->rdataset) &&
	    WANTDNSSEC(qctx->client))
	{
		if (!qctx->fname->name.attributes.wildcard) {
			dns_name_t *found;
			dns_name_t *qname;
			dns_fixedname_t fixed;
			isc_buffer_t b;
			     ...
			     qctx->qtype == dns_rdatatype_ds))
			{
				...
				query_findclosestnsec3(
					...
					qctx->client, qctx->rdataset,
					qctx->sigrdataset, &qctx->fname->name, false,
					NULL);
			}
		} else {
			ns_client_releasename(qctx->client, &qctx->fname);
...
query_addnxrrsetnsec(query_ctx_t *qctx) {
	...
	labels = dns_name_countlabels(&qctx->fname->name);
	...
	dns_name_split(&qctx->fname->name, sig.labels + 1, NULL, &fname->name);
	/* This will succeed, since we've stripped labels. */
	RUNTIME_CHECK(dns_name_concatenate(dns_wildcardname, &fname->name, &fname->name) ==
		      ISC_R_SUCCESS);
...
query_redirect(query_ctx_t *qctx, isc_result_t saved_result) {
	...
	result = redirect(qctx->client, &qctx->fname->name, qctx->rdataset,
	...
	result = redirect2(qctx->client, &qctx->fname->name, qctx->rdataset,
			   &qctx->node, &qctx->db, &qctx->version, qctx->type,
			   &qctx->is_zone);
	switch (result) {
	case ISC_R_SUCCESS:
	...
	case DNS_R_CONTINUE:
		...
		dns_name_copy(&qctx->fname->name, qctx->client->query.redirect.fname);
		qctx->client->query.redirect.authoritative =
			qctx->authoritative;
...
query_synthnodata(query_ctx_t *qctx, const dns_name_t *signer,
	...
	dns_name_copy(signer, &name->name);
...
query_synthwildcard(query_ctx_t *qctx, dns_rdataset_t *rdataset,
	...
	dbuf = ns_client_getnamebuf(qctx->client);
	name = ns_client_newname(qctx->client, dbuf, &b);
	dns_name_copy(&qctx->client->query.qname->name, &name->name);
...
query_synthcnamewildcard(query_ctx_t *qctx, dns_rdataset_t *rdataset,
			 dns_rdataset_t *sigrdataset) {
	isc_result_t result;
	dns_name_with_links_t *tname = NULL;
	dns_rdata_t rdata = DNS_RDATA_INIT;
	dns_rdata_cname_t cname;
	 ...
	 */
	{
		dns_name_with_links_t *tname_wl = NULL;
		dns_message_gettempname(qctx->client->message, &tname_wl);
		tname = tname_wl;
	}

	result = dns_rdataset_first(rdataset);
	if (result != ISC_R_SUCCESS) {
		dns_name_with_links_t *tname_wl = tname;
		dns_message_puttempname(qctx->client->message, &tname_wl);
		tname = NULL;
		return result;
	}
	...
	if (dns_name_equal(&qctx->client->query.qname->name, &cname.cname)) {
		dns_name_with_links_t *tname_wl = tname;
		dns_message_puttempname(qctx->client->message, &tname_wl);
		tname = NULL;
	...
	dns_name_copy(&cname.cname, &tname->name);
...
query_synthnxdomainnodata(query_ctx_t *qctx, bool nodata, dns_name_t *nowild,
	...
	if (WANTDNSSEC(qctx->client)) {
		...
		dbuf = ns_client_getnamebuf(qctx->client);
		name = ns_client_newname(qctx->client, dbuf, &b);
		dns_name_copy(nowild, &name->name);
...
query_coveringnsec(query_ctx_t *qctx) {
	 ...
	 */
	dns_name_clone(&qctx->client->query.qname->name, &qname);
	...
	if (!dns_name_issubdomain(&qctx->fname->name, namespace)) {
		goto cleanup;
	}
	...
	fixedname = dns_fixedname_initname(&fixed);
	 ...
	 */
	result = dns_nsec_noexistnodata(qctx->qtype, &qctx->client->query.qname->name,
					&qctx->fname->name, qctx->rdataset, &exists,
	...
	if (exists) {
		...
		CHECK(dns_db_findext(db, signer, qctx->version,
				     ...
				     qctx->client->inner.now, &node, fixedname, &cm,
	...
	CHECK(dns_db_findext(db, signer, qctx->version, dns_rdatatype_soa,
			     dboptions, qctx->client->inner.now, &node, fixedname,
...
query_ncache(query_ctx_t *qctx, isc_result_t result) {
	...
	if (result == DNS_R_NCACHENXDOMAIN) {
		...
		if (qctx->qtype == dns_rdatatype_ptr &&
		    qctx->client->message->rdclass == dns_rdataclass_in &&
		    dns_name_countlabels(&qctx->fname->name) == 7)
		{
			warn_rfc1918(qctx->client, &qctx->fname->name, qctx->rdataset);
		}
	}
...
query_zerottl_refetch(query_ctx_t *qctx) {
	...
	result = ns_query_recurse(qctx->client, qctx->qtype,
				  &qctx->client->query.qname->name, NULL, NULL,
				  qctx->resuming);
	if (result == ISC_R_SUCCESS) {
...
query_cname(query_ctx_t *qctx) {
	...
	if (WANTDNSSEC(qctx->client) && qctx->fname->name.attributes.wildcard) {
	...
	if (!qctx->is_zone && RECURSIONOK(qctx->client)) {
		query_prefetch(qctx->client, &qctx->fname->name, qctx->rdataset);
	}
...
query_dname(query_ctx_t *qctx) {
	 ...
	 */
	namereln = dns_name_fullcompare(&qctx->client->query.qname->name, &qctx->fname->name,
					&order, &nlabels);
	...
	dns_name_copy(&dname.dname, &tname->name);
	...
	dns_name_split(&qctx->client->query.qname->name, nlabels, prefix, NULL);
	INSIST(qctx->fname == NULL);
	qctx->dbuf = ns_client_getnamebuf(qctx->client);
	qctx->fname = ns_client_newname(qctx->client, qctx->dbuf, &b);
	result = dns_name_concatenate(prefix, &tname->name, &qctx->fname->name);
	dns_message_puttempname(qctx->client->message, &tname);
	tname = NULL;
...
query_addcname(query_ctx_t *qctx, dns_trust_t trust, dns_ttl_t ttl) {
	...
	isc_region_t r;
	dns_name_with_links_t *aname = NULL;

	dns_message_gettempname(client->message, &aname);

	dns_name_copy(&client->query.qname->name, &aname->name);
	...
	dns_name_toregion(&qctx->fname->name, &r);
	rdata->data = r.base;
	...
	dns_rdataset_setownercase(rdataset, &aname->name);
...
query_addsoa(query_ctx_t *qctx, unsigned int override_ttl,
	 ...
	 */
	dns_name_clone(dns_db_origin(qctx->db), &name->name);
	...
	if (result == ISC_R_SUCCESS) {
		...
		result = dns_db_findext(qctx->db, &name->name, qctx->version,
					dns_rdatatype_soa,
...
query_addns(query_ctx_t *qctx) {
	...
	if (result == ISC_R_SUCCESS) {
		...
		result = dns_db_findext(qctx->db, &name->name, NULL,
					dns_rdatatype_ns,
					client->query.dboptions, 0, &node,
					fname, &cm, &ci, rdataset, sigrdataset);
...
query_addbestns(query_ctx_t *qctx) {
	...
	dns_dbnode_t *node = NULL;
	dns_name_with_links_t *fname = NULL, *zfname = NULL;
	dns_rdataset_t *rdataset = NULL, *sigrdataset = NULL;
	...
	dns_name_init(&qname);
	dns_name_clone(&client->query.qname->name, &qname);
...
db_find:
	...
	if (is_zone) {
		result = dns_db_findext(
			db, &client->query.qname->name, version, dns_rdatatype_ns,
			client->query.dboptions, client->inner.now, &node,
			&fname->name, &cm, &ci, rdataset, sigrdataset);
		if (result != DNS_R_DELEGATION) {
			goto cleanup;
		}
		if (USECACHE(client)) {
		...
		result = dns_db_findzonecut(db, &client->query.qname->name,
					    client->query.dboptions,
					    client->inner.now, &node, &fname->name,
					    NULL, rdataset, sigrdataset);
		if (result == ISC_R_SUCCESS) {
			if (zfname != NULL &&
			    !dns_name_issubdomain(&fname->name, &zfname->name))
			{
				/*
	...
	if ((DNS_TRUST_PENDING(rdataset->trust) ||
	    ...
	    !validate(client, db, &fname->name, rdataset, sigrdataset) &&
...
again:
	...
	result = dns_db_findext(qctx->db, name, qctx->version,
				dns_rdatatype_nsec, options, 0, &node, &fname->name,
	...
	if (!dns_rdataset_isassociated(rdataset)) {
		...
		unsigned int minlabels = dns_name_countlabels(&fname->name);
		...
		query_findclosestnsec3(cname, qctx->db, qctx->version, client,
				       rdataset, sigrdataset, &fname->name, true,
				       cname);
		if (!dns_rdataset_isassociated(rdataset)) {
			goto cleanup;
		}
		...
		query_findclosestnsec3(wname, qctx->db, qctx->version, client,
				       rdataset, sigrdataset, &fname->name, false,
				       NULL);
		if (!dns_rdataset_isassociated(rdataset)) {
			goto cleanup;
		}
		...
		query_findclosestnsec3(wname, qctx->db, qctx->version, client,
				       rdataset, sigrdataset, &fname->name, nodata,
				       NULL);
		if (!dns_rdataset_isassociated(rdataset)) {
			goto cleanup;
		}
		...
		if (result == ISC_R_SUCCESS) {
			...
			RUNTIME_CHECK(result == ISC_R_SUCCESS);
			(void)dns_name_fullcompare(name, &fname->name, &order,
						   &olabels);
...
query_glueanswer(query_ctx_t *qctx) {
	...
	MSG_SECTION_FOREACH(msg, section, name) {
		if (dns_name_equal(&name->name, &qctx->client->query.qname->name)) {
...
log_tat(ns_client_t *client) {
	...
	if ((client->query.qtype != dns_rdatatype_null ||
	     !dns_name_istat(&client->query.qname->name)) &&
...
log_queryerror(ns_client_t *client, isc_result_t result, int line, int level) {
	 ...
	 */
	if (client->query.origqname != NULL) {
		dns_name_format(&client->query.origqname->name, namebuf,
				sizeof(namebuf));
		namep = namebuf;
		sep1 = " for ";
