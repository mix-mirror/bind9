 ...
 */
#define TCPCLIENT(client) (((client)->inner.attributes & NS_CLIENTATTR_TCP) != 0)
				   ...
				   update_class == dns_rdataclass_none)
			{
				...
				if (flag &&
				    !dns_ssutable_checkrules(
					    ssutable, client->inner.signer, name,
				...
				if (!ssu_checkall(db, ver, name, ssutable,
						  client->inner.signer, &netaddr, env,
...
ns_update_start(ns_client_t *client, isc_nmhandle_t *handle,
	...
	result = dns_view_findzone(client->inner.view, zonename, DNS_ZTFIND_EXACT,
				   &zone);
	if (result != ISC_R_SUCCESS) {
