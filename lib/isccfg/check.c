...
check_orderent(const cfg_obj_t *ent) {
	...
	if (cfg_obj_isstring(obj)) {
		...
		tresult = dns_fixedname_fromtext(&fixed, &b, dns_rootname, 0);
		if (tresult != ISC_R_SUCCESS) {
			cfg_obj_log(obj, ISC_LOG_ERROR,
...
check_dual_stack(const cfg_obj_t *options) {
	...
	CFG_LIST_FOREACH(obj, element) {
		...
		dns_fixedname_init(&fixed);
		tresult = dns_fixedname_fromtext(&fixed, &buffer, dns_rootname,
						 0);
		if (tresult != ISC_R_SUCCESS) {
...
check_name(const char *str) {
	...
	dns_fixedname_init(&fixed);
	return dns_fixedname_fromstring(&fixed, str, dns_rootname, 0);
}
...
lookup_key(const cfg_obj_t *config, const dns_name_t *keyname) {
	...
	CFG_LIST_FOREACH(keys, elt) {
		...
		isc_result_t result = dns_fixedname_fromstring(
			&foundfname, foundkeystr, dns_rootname, 0);
...
validate_remotes_key(const cfg_obj_t *voptions, const cfg_obj_t *config,
	...
	if (cfg_obj_isstring(key)) {
		...
		result = dns_fixedname_fromstring(&fname, str, dns_rootname, 0);
		if (result != ISC_R_SUCCESS) {
...
check_update_policy(const cfg_obj_t *policy) {
	...
	CFG_LIST_FOREACH(policy, element) {
		...
		tresult = dns_fixedname_fromstring(&fixed_id, str, dns_rootname,
						   1);
		if (tresult != ISC_R_SUCCESS) {
			cfg_obj_log(identity, ISC_LOG_ERROR,
				    "'%s' is not a valid name", str);
		...
		if (tresult == ISC_R_SUCCESS) {
			tresult = dns_fixedname_fromstring(&fixed_name, str,
							   dns_rootname, 0);
			if (tresult != ISC_R_SUCCESS) {
...
isccfg_check_zoneconf(const cfg_obj_t *zconfig, const cfg_obj_t *voptions,
	...
	tresult = dns_fixedname_fromtext(&fixedname, &b, dns_rootname,
					 DNS_NAME_DOWNCASE);
	if (tresult != ISC_R_SUCCESS) {
		cfg_obj_log(zconfig, ISC_LOG_ERROR,
	...
	if (zname != NULL && ztype == CFG_ZONE_STATICSTUB && obj != NULL) {
		CFG_LIST_FOREACH(obj, element) {
			...
			tresult = dns_fixedname_fromtext(&fixed_sname, &b2,
							 dns_rootname, 0);
			if (tresult != ISC_R_SUCCESS) {
				cfg_obj_log(zconfig, ISC_LOG_ERROR,
	...
	if (obj != NULL) {
		...
		tresult = dns_fixedname_fromstring(&fad, str, dns_rootname, 0);
		if (tresult != ISC_R_SUCCESS) {
			cfg_obj_log(obj, ISC_LOG_ERROR,
				    "'%s' is not a valid name", str);
			if (result == ISC_R_SUCCESS) {
...
check_keylist(const cfg_obj_t *keys, isc_symtab_t *symtab, isc_mem_t *mctx) {
	...
	CFG_LIST_FOREACH(keys, element) {
		...
		tresult = dns_fixedname_fromtext(&fname, &b, dns_rootname, 0);
		if (tresult != ISC_R_SUCCESS) {
			cfg_obj_log(key, ISC_LOG_ERROR,
...
check_servers(const cfg_obj_t *config, const cfg_obj_t *voptions,
	...
	CFG_LIST_FOREACH(servers, e1) {
		...
		if (keys != NULL) {
			...
			tresult = dns_fixedname_fromtext(&fname, &b,
							 dns_rootname, 0);
			if (tresult != ISC_R_SUCCESS) {
...
check_trust_anchor(const cfg_obj_t *key, unsigned int *flagsp) {
	...
	static const unsigned char root_ksk_2026[] = {
		
	};
	...
	static const unsigned char root_ds_2_2026[] = {
	...
	case STATIC_DNSKEY:
		...
		if (result == ISC_R_SUCCESS && dns_name_isroot(keyname)) {
			...
			*flagsp |= (managed ? ROOT_KSK_MANAGED
			...
			if (rdata1 == 257 && rdata2 == 3 && rdata3 == 8 &&
			    (isc_buffer_usedlength(&b) ==
			     sizeof(root_ksk_2026)) &&
			    memcmp(data, root_ksk_2026,
				   sizeof(root_ksk_2026)) == 0)
			{
				*flagsp |= ROOT_KSK_2026;
			}
		}
		break;
	...
	case STATIC_DS:
		...
		if (result == ISC_R_SUCCESS && dns_name_isroot(keyname)) {
			...
			*flagsp |= (managed ? ROOT_KSK_MANAGED
			...
			if (rdata1 == 38696 && rdata2 == 8 && rdata3 == 2 &&
			    (isc_buffer_usedlength(&b) ==
			     sizeof(root_ds_2_2026)) &&
			    memcmp(data, root_ds_2_2026,
				   sizeof(root_ds_2_2026)) == 0)
			{
				*flagsp |= ROOT_KSK_2026;
			}
		}
...
record_static_keys(isc_symtab_t *symtab, isc_mem_t *mctx,
	...
	CFG_LIST_FOREACH(keylist, elt) {
		...
		result = dns_fixedname_fromstring(&fixed, str, dns_rootname, 0);
		if (result != ISC_R_SUCCESS) {
			/*
...
check_rpz_catz(const char *rpz_catz, const cfg_obj_t *rpz_obj,
	...
	CFG_LIST_FOREACH(obj, element) {
		...
		tresult = dns_fixedname_fromstring(&fixed, zonename,
						   dns_rootname, 0);
		if (tresult != ISC_R_SUCCESS) {
			cfg_obj_log(nameobj, ISC_LOG_ERROR,
	...
	for (i = 0; i < 2; i++) {
		if (check_keys[i] != NULL) {
			    ...
			    (taflags & ROOT_KSK_2026) == 0)
			{
				cfg_obj_log(check_keys[i], ISC_LOG_WARNING,
					    "initial-key entry for the root "
					    "zone uses the 2017 key without "
					    "the updated 2026 key");
			}

			/*
			 * Warn about missing 2017 trust anchor until
			 * 23:59 October 11 2026 UTC.  After this the IANA root
			 * zone is expected to be signed signed with the 2026
			 * key.
			 */
			if (time(NULL) < 1791763140 &&
			    (taflags & ROOT_KSK_2017) == 0 &&
			    (taflags & ROOT_KSK_2026) != 0)
			{
				cfg_obj_log(check_keys[i], ISC_LOG_WARNING,
					    "initial-key entry for the root "
					    "zone uses the updated 2026 key "
					    "without the 2017 key");
			}
