...
constexpr uint32_t preferred_versions[] = {
	NGTCP2_PROTO_VER_V2,
	NGTCP2_PROTO_VER_V1,
};
 ...
 */
constexpr uint8_t client_label[] = {
	'c', 'l', 'i', 'e', 'n', 't', ' ', 'i', 'n',
};
 ...
 * The value is specified in RFC9001, Section 5.2.
 */
constexpr uint8_t server_label[] = {
	's', 'e', 'r', 'v', 'e', 'r', ' ', 'i', 'n',
};
 ...
 */
constexpr uint8_t key_label_v1[] = {
	'q', 'u', 'i', 'c', ' ', 'k', 'e', 'y',
};
 ...
 */
constexpr uint8_t key_update_label_v1[] = {
	'q', 'u', 'i', 'c', ' ', 'k', 'u',
};
 ...
 */
constexpr uint8_t key_label_v2[] = {
	'q', 'u', 'i', 'c', 'v', '2', ' ', 'k', 'e', 'y',
};
 ...
 * The value is specified in RFC9369, Section 3.3.2.
 */
constexpr uint8_t key_update_label_v2[] = {
	'q', 'u', 'i', 'c', 'v', '2', ' ', 'k', 'u',
};
...
static int
quic_alpn_select(isc_tls_t *tls ISC_ATTR_UNUSED, const unsigned char **outp,
...
common_settings(ngtcp2_settings *settings) {
	 ...
	 */
	settings->no_pmtud = 1;
	settings->handshake_timeout = 100000;
...
/*
 * ngtcp2_mem <-> isc_mem bridge
 */

/**
 * \brief
 * Bridge ngtcp2_mem to isc_mem_allocate.
 */
static void *
...
/*
 * `ngtcp2_callbacks` implementation.
 */
...
recv_stream_data_cb(ngtcp2_conn *ngconn ISC_ATTR_UNUSED, uint32_t flags,
	...
	if (conn->cb != NULL && conn->cb->data_read != NULL) {
		conn->cb->data_read(conn->cbarg, info,
...
get_new_connection_id2_cb(ngtcp2_conn *ngconn ISC_ATTR_UNUSED,
	...
	for (;;) {
		...
		result = isc_quic_router_add_cid(
			conn->router, (isc_constregion_t){ buffer, cidlen },
			conn);
		if (result == ISC_R_SUCCESS) {
			ngcid->datalen = cidlen;
			memmove(ngcid->data, buffer, cidlen);
...
dcid_status2_cb(ngtcp2_conn *ngconn ISC_ATTR_UNUSED,
	...
	case NGTCP2_CONNECTION_ID_STATUS_TYPE_ACTIVATE:
		result = isc_quic_router_add_cid(conn->router, cid, conn);
		if (result == ISC_R_SUCCESS && ngtoken != NULL) {
			result = isc_quic_router_add_stateless_reset(
				conn->router, ngtoken->data, conn);
		}
		break;
...
isc_quic_conn_client_create(isc_mem_t *mctx, isc_quic_router_t *router,
	...
	REQUIRE(options != NULL && options->idle_timeout != 0 &&
	...
	*conn = (isc_quic_conn_t){
	...
	if (options->alpn.base != NULL) {
		...
		SSL_set_alpn_protos(tls, conn->alpn.data, conn->alpn.len);
	}

	isc__quic_setup_tls(tls, conn);
...
isc_quic_conn_server_create(isc_mem_t *mctx, isc_quic_router_t *router,
	...
	*conn = (isc_quic_conn_t){
	...
	if (ngtcp2_conn_server_new(&conn->inner, &dcid, &scid, &path,
