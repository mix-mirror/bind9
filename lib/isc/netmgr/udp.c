...
udp_listener_destroy(isc_nm_udplistener_t *listener) {
	...
	isc_mem_cput(mctx, listener->children, listener->nchildren,
...
void
isc_nm_udplistener_attach(isc_nm_udplistener_t *source,
			  isc_nm_udplistener_t **targetp) {
	REQUIRE(VALID_UDP_LISTENER(source));
	REQUIRE(targetp != NULL && *targetp == NULL);

	isc_refcount_increment0(&source->references);
	*targetp = source;
}

void
isc_nm_udplistener_detach(isc_nm_udplistener_t **listenerp) {
	isc_nm_udplistener_t *listener = NULL;

	REQUIRE(listenerp != NULL && *listenerp != NULL);
	REQUIRE(VALID_UDP_LISTENER(*listenerp));

	listener = *listenerp;
	*listenerp = NULL;
	if (isc_refcount_decrement(&listener->references) == 1) {
		udp_listener_destroy(listener);
	}
}
...
start_udp_child_job(void *arg) {
	...
	sock->fd = isc__netmgr->load_balance_sockets
			   ? isc__nm_udp_lb_socket(
...
isc_nm_listenudp(uint32_t workers, isc_sockaddr_t *iface, isc_nm_recv_cb_t cb,
	...
	listener = isc_mem_get(worker->mctx, sizeof(*listener));
	*listener = (isc_nm_udplistener_t){
	...
	isc_mem_attach(worker->mctx, &listener->mctx);
	listener->children = isc_mem_cget(listener->mctx, listener->nchildren,
					  sizeof(listener->children[0]));
