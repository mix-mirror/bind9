...
isc_urcu_malloc(void *state, size_t size) {
	REQUIRE(state != NULL);
	isc_mem_t *mctx = NULL;
	isc_mem_attach(state, &mctx);
	return isc_mem_allocate(mctx, size);
}
...
isc_urcu_calloc(void *state, size_t nmemb, size_t size) {
	...
	isc_mem_t *mctx = NULL;
	isc_mem_attach(state, &mctx);
	return isc_mem_callocate(mctx, nmemb, size);
}
...
isc__urcu_initialize(void) {
	isc_mem_create(&isc__urcu_mctx);
	isc_mem_setname(isc__urcu_mctx, "cds_lfth_alloc");
