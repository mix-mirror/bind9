...
/*%<
 * Set a callback receiving allocation and deallocation byte deltas.
 * The context must be empty and callers must exclude concurrent allocations
 * and deallocations. The callback data must remain valid until the callback
 * is cleared, or the context is destroyed.
 */
