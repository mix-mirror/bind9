...
cache_delegns(respctx_t *rctx) {
	...
	DNS_RDATASET_FOREACH(rctx->ns_rdataset) {
		...
		/*
		 * TODO: Currently the nameserver name is added regardless
		 * of whether there are glues or not. What should be done
		 * instead is to add the nameserver name _only_ if no
...
resume_dslookup(void *arg) {
	...
	case ISC_R_SUCCESS:
		 ...
		 * And because delegset is NULL, various things go wrong (i.e.
