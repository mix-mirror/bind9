...
cache_delegns(respctx_t *rctx) {
	...
	//	dns_deleg_dump(delegdb, NULL, 0, &b);
	//	fprintf(stderr,
	//		"------------------------DELEG------------------------\n"
	//		"%s\n"
	//		"-----------------------------------------------------\n",
...
rctx_answer_match(respctx_t *rctx) {
	 ...
	 * resolutions below bar.foo.example will still works, because the
	 ...
	 *
	 * BTW, not storing it will broke 2 system tests:
	 *
	 *   test_insecure_proof_optout
	 *   test_insecure_proof_nsec3
	 *
	 * But only because minimal-response is disabled, at there is a full
	 * answer comparaison at some point. Without this delegation, we are
	 * missing a glue for bar.foo.example so it won't be part of the
	 * additional sections of the responses (but in contextes where this is
	 * not needed anyway). So it's easy to remove the additional check if we
	 * decide we shouldn't store the delegation here.
	 *
	 * (Of course, if we decide to keep this, I wouldn't "hack" using
