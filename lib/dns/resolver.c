 ...
 * the resolver doesn't really supports DELEG so it doesn't matter, but once it
 * get supported, this code must somehow bails off if there is already a
...
cache_delegns(respctx_t *rctx) {
	...
	DNS_RDATASET_FOREACH(rctx->ns_rdataset) {
		...
		/*
		 * TODO: Currently the nameserver name is added
		 * independently if there are glues or not. What should be
		 * done instead, is to add the nameserver name _only_ if no
		 * A/AAAA glues are found for this name. Otherwise, the
		 * resolver would have to resolve the name for nothing.
		 * (Because it has the IPs for this server, so it will be
		 * queried anyway.)
		 *
		 * (Currently, I left it that way because it makes it
		 * easier to test with minimal changes in the resolver
		 * `fctx_getnameserver()` logic, but this will need to
		 * change ASAP.)
		 */
