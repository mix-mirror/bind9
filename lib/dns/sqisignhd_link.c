...
STATIC_ASSERT(OQS_SIG_sqisign_lvl1_length_public_key == DNS_KEY_SQISIGNSIZE,
	      "SQIsign public key size mismatch");
STATIC_ASSERT(OQS_SIG_sqisign_lvl1_length_secret_key == DNS_SEC_SQISIGNSIZE,
	      "SQIsign secret key size mismatch");
STATIC_ASSERT(OQS_SIG_sqisign_lvl1_length_signature == DNS_SIG_SQISIGNSIZE,
	      "SQIsign signature size mismatch");

/*
 * Wire format "q." — SQIsign with NIST-I parameter set, prepended to
 * the public key and signature material as specified for PRIVATEDNS
 * (RFC 4034 appendix A.1.1); see
 * https://github.com/oerdnj/dnssec-algorithm-testing-registry
 */
static unsigned char sqisign_dns[] = "\001q";
...
dst__sqisign_sign(dst_context_t *dctx, isc_buffer_t *sig) {
	...
	if (sigreg.length < sizeof(sqisign_dns) + DNS_SIG_SQISIGNSIZE) {
		result = ISC_R_NOSPACE;
		goto done;
	}

	/*
	 * Add the PRIVATEDNS identifier to the start of the signature.
	 */
	isc_buffer_putmem(sig, sqisign_dns, sizeof(sqisign_dns));
	isc_region_consume(&sigreg, sizeof(sqisign_dns));

	isc_buffer_usedregion(buf, &tbsreg);

	OQS_STATUS status = OQS_SIG_sqisign_lvl1_sign(
		sigreg.base, &siglen, tbsreg.base, tbsreg.length,
		key->keydata.keypair.priv);
	if (status != OQS_SUCCESS) {
		result = DST_R_SIGNFAILURE;
		isc_log_write(dctx->category, DNS_LOGMODULE_CRYPTO,
			      ISC_LOG_WARNING,
			      "OQS_SIG_sqisign_lvl1_sign (%s:%d) failed (%s)",
			      __FILE__, __LINE__, isc_result_totext(result));
		goto done;
	}
	INSIST(siglen == DNS_SIG_SQISIGNSIZE);
...
dst__sqisign_verify(dst_context_t *dctx, const isc_region_t *sigreg) {
	...
	isc_buffer_t *buf = dctx->ctxdata.generic;
	isc_region_t tbsreg;
	isc_region_t r = *sigreg;

	/*
	 * Check the PRIVATEDNS identifier at the start of the signature.
	 */
	if (r.length != sizeof(sqisign_dns) + DNS_SIG_SQISIGNSIZE ||
	    memcmp(r.base, sqisign_dns, sizeof(sqisign_dns)) != 0)
	{
		result = DST_R_VERIFYFAILURE;
		goto done;
	}
	isc_region_consume(&r, sizeof(sqisign_dns));

	isc_buffer_usedregion(buf, &tbsreg);

	OQS_STATUS status =
		OQS_SIG_sqisign_lvl1_verify(tbsreg.base, tbsreg.length, r.base,
					    r.length, key->keydata.keypair.pub);
	if (status != OQS_SUCCESS) {
		result = DST_R_VERIFYFAILURE;
		/* FIXME: Is it really a warning if the verification fails */
		isc_log_write(dctx->category, DNS_LOGMODULE_CRYPTO,
			      ISC_LOG_WARNING,
			      "OQS_SIG_sqisign_lvl1_verify (%s:%d) failed (%s)",
			      __FILE__, __LINE__, isc_result_totext(result));
		goto done;
	}
...
dst__sqisign_generate(dst_key_t *key, int unused ISC_ATTR_UNUSED,
	...
	OQS_STATUS status = OQS_SIG_sqisign_lvl1_keypair(pk, sk);
	if (status != OQS_SUCCESS) {
		result = DST_R_CRYPTOFAILURE;
		isc_log_write(
			DNS_LOGCATEGORY_GENERAL, DNS_LOGMODULE_CRYPTO,
			ISC_LOG_WARNING,
			"OQS_SIG_sqisign_lvl1_keypair (%s:%d) failed (%s)",
			__FILE__, __LINE__, isc_result_totext(result));
		goto done;
	}
...
dst__sqisign_todns(const dst_key_t *key, isc_buffer_t *data) {
	...
	if (r.length < sizeof(sqisign_dns) + DNS_KEY_SQISIGNSIZE) {
		return ISC_R_NOSPACE;
	}

	/*
	 * Add the PRIVATEDNS identifier to the front of the public key
	 * material.
	 */
	isc_buffer_putmem(data, sqisign_dns, sizeof(sqisign_dns));
...
dst__sqisign_fromdns(dst_key_t *key, isc_buffer_t *data) {
	...
	/*
	 * Check the PRIVATEDNS identifier in front of the public key
	 * material.
	 */
	if (r.length != sizeof(sqisign_dns) + DNS_KEY_SQISIGNSIZE ||
	    memcmp(r.base, sqisign_dns, sizeof(sqisign_dns)) != 0)
	...
	key->key_size = DNS_KEY_SQISIGNSIZE * 8;
	isc_buffer_forward(data, sizeof(sqisign_dns) + DNS_KEY_SQISIGNSIZE);
