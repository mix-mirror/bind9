...
static thread_local detachrefs_t cfgmgr_refstodetach;
...
static MDB_env *cfgmgr_env = NULL;
static thread_local context_t cfgmgr_ctx = (context_t){};
...
/*
 * dns_cfgmgr_foreach API is not re-entrant (it was previously, by using its own
...
static bool
cfgmgr_openlmdbcursor(const char *dbname, MDB_cursor **cursor,
			   MDB_dbi *dbi, bool readonly) {
	int result;

	result = mdb_dbi_open(cfgmgr_ctx.txn, dbname,
			      readonly ? 0 : MDB_CREATE, dbi);
	if (result != 0) {
		return false;
	}

	result = mdb_cursor_open(cfgmgr_ctx.txn, *dbi, cursor);
	if (result != 0) {
		return false;
	}
...
static void
cfgmgr_closelmdbcursors(void) {
	mdb_cursor_close(cfgmgr_ctx.builtincursor);
	cfgmgr_ctx.builtincursor = NULL;

	mdb_cursor_close(cfgmgr_ctx.usercursor);
	cfgmgr_ctx.usercursor = NULL;

	mdb_cursor_close(cfgmgr_ctx.runningcursor);
	cfgmgr_ctx.runningcursor = NULL;
}

static bool
cfgmgr_startlmdbtransaction(bool readonly) {
	if (mdb_txn_begin(cfgmgr_env, NULL, readonly ? MDB_RDONLY : 0,
			  &cfgmgr_ctx.txn) != 0)
	{
		return false;
	}

	INSIST(cfgmgr_ctx.txn != NULL);

	if (!cfgmgr_openlmdbcursor(
		    "builtin", &cfgmgr_ctx.builtincursor,
		    &cfgmgr_ctx.builtindbi, readonly) ||
	    !cfgmgr_openlmdbcursor("user", &cfgmgr_ctx.usercursor,
					&cfgmgr_ctx.userdbi, readonly) ||
	    !cfgmgr_openlmdbcursor("running",
					&cfgmgr_ctx.runningcursor,
					&cfgmgr_ctx.runningdbi, readonly))
	{
		cfgmgr_closelmdbcursors();
		mdb_txn_abort(cfgmgr_ctx.txn);
		cfgmgr_ctx.txn = NULL;
...
dns_cfgmgr_lmdb_init(isc_mem_t *mctx) {
	...
	/*
	 * really just for testing... Should be named run dir.
	 */
	const char dbpath[] = "/tmp/";

	REQUIRE(cfgmgr_mctx == NULL);
	REQUIRE(cfgmgr_env == NULL);
	REQUIRE(mctx != NULL);
	...
	REQUIRE(sizeof(dns_cfgmgr_type_t) == sizeof(uint8_t));

	isc_mem_attach(mctx, &cfgmgr_mctx);
	INSIST(cfgmgr_mctx != NULL);

	result = mdb_env_create(&cfgmgr_env);
	if (result != 0) {
		result = ISC_R_FAILURE;
		goto cleanup;
	}

	INSIST(cfgmgr_env != NULL);
	random = isc_random32();
	 ...
	 */
	cfgmgr_snprintf(dbname, BUFLEN, "%s-%" PRIu32, dbpath, random);
	cfgmgr_snprintf(dblockname, BUFLEN, "%s-%" PRIu32 "-lock", dbpath,
			     random);

	result = mdb_env_set_maxdbs(cfgmgr_env, 3);
	if (result != 0) {
		result = ISC_R_FAILURE;
		goto cleanup;
	}
	...
	result = mdb_env_open(cfgmgr_env, dbname, openflags, 0600);
	if (result != 0) {
		result = ISC_R_FAILURE;
		goto cleanup;
	}
...
static void
cfgmgr_requirenotxn(void) {
	REQUIRE(cfgmgr_env != NULL);
	REQUIRE(cfgmgr_ctx.txn == NULL);
	REQUIRE(cfgmgr_ctx.builtincursor == NULL);
	REQUIRE(cfgmgr_ctx.usercursor == NULL);
	REQUIRE(cfgmgr_ctx.runningcursor == NULL);
}
...
static MDB_cursor *
cfgmgr_dbcursor(void) {
	switch (cfgmgr_ctx.mode) {
	case DNS_CFGMGR_MODEBUILTIN:
		return cfgmgr_ctx.builtincursor;
	case DNS_CFGMGR_MODEUSER:
		return cfgmgr_ctx.usercursor;
	case DNS_CFGMGR_MODERUNNING:
		return cfgmgr_ctx.runningcursor;
	}
...
static isc_result_t
cfgmgr_dbget(MDB_cursor *cursor, const char *k, void **v, size_t *vlen) {
...
static isc_result_t
cfgmgr_dbput(MDB_cursor *cursor, const char *k, void *v, size_t vlen) {
	MDB_val dbk = { .mv_size = strlen(k) + 1, .mv_data = (void *)k };
	MDB_val dbv = { .mv_size = vlen, .mv_data = v };
	int r = mdb_cursor_put(cursor, &dbk, &dbv, 0);  

	return r == MDB_SUCCESS ? ISC_R_SUCCESS : ISC_R_FAILURE;
}

static void
cfgmgr_dbdel(MDB_cursor *cursor) {
	REQUIRE(mdb_cursor_del(cursor, 0) == MDB_SUCCESS);
}

static isc_result_t
cfgmgr_dbgetnext(MDB_cursor *cursor, const char *path,
...
cfgmgr_setlasterror(const char *fmt, ...) {
	...
	va_start(ap, fmt);
	REQUIRE(vsnprintf(cfgmgr_lasterror, BUFLEN, fmt, ap) < BUFLEN);
	va_end(ap);
}

static void
cfgmgr_requiretxn(void) {
	REQUIRE(cfgmgr_env != NULL);
	REQUIRE(cfgmgr_ctx.txn != NULL);
	REQUIRE(cfgmgr_ctx.builtincursor != NULL);
	REQUIRE(cfgmgr_ctx.usercursor != NULL);
...
static void
cfgmgr_requirerotxn(void) {
	cfgmgr_requiretxn();
	REQUIRE(cfgmgr_ctx.readonly);
}

static void
cfgmgr_requirerwtxn(void) {
	cfgmgr_requiretxn();
	REQUIRE(!cfgmgr_ctx.readonly);
}
...
dns_cfgmgr_lmdb_mode(dns_cfgmgr_mode_t mode) {
	...
	REQUIRE(cfgmgr_builtininitialized ||
		mode == DNS_CFGMGR_MODEBUILTIN);
	REQUIRE(cfgmgr_userinitialized || mode == DNS_CFGMGR_MODEBUILTIN ||
		mode == DNS_CFGMGR_MODEUSER);

	cfgmgr_ctx.mode = mode;
}

static isc_result_t
cfgmgr_opentransaction(bool readonly) {
	cfgmgr_requirenotxn();
	cfgmgr_setlasterror("");

	if (!cfgmgr_startlmdbtransaction(readonly)) {
		return ISC_R_FAILURE;
	}

	cfgmgr_ctx.readonly = readonly;

	if (readonly) {
		cfgmgr_requirerotxn();
	} else {
		cfgmgr_requirerwtxn();
		ISC_LIST_INIT(cfgmgr_refstodetach);
	}
...
dns_cfgmgr_lmdb_txn(void) {
	REQUIRE(cfgmgr_builtininitialized);
	REQUIRE(cfgmgr_ctx.mode == DNS_CFGMGR_MODEBUILTIN ||
		cfgmgr_userinitialized);

	INSIST(cfgmgr_opentransaction(true) == ISC_R_SUCCESS);
}

void
dns_cfgmgr_lmdb_closetxn(void) {
	cfgmgr_requirerotxn();
	cfgmgr_rollbacklmdbtxn();
	cfgmgr_requirenotxn();
}
...
dns_cfgmgr_lmdb_rwtxn(void) {
	if (!cfgmgr_builtininitialized) {
		...
		REQUIRE(cfgmgr_ctx.mode == DNS_CFGMGR_MODEUSER);
	} else {
		REQUIRE(cfgmgr_ctx.mode == DNS_CFGMGR_MODERUNNING);
	}
	INSIST(cfgmgr_opentransaction(false) == ISC_R_SUCCESS);
}

static bool
cfgmgr_materialize_cb(void *state, const char *nextkey, size_t nextkeylen,
			   const void *nextval, size_t nextvallen) {
	UNUSED(state);
	UNUSED(nextkeylen);
	REQUIRE(cfgmgr_dbput(cfgmgr_ctx.runningcursor, nextkey,
				  (void *)nextval,
...
cfgmgr_materialize(void) {
	cfgmgr_dbgetnext(cfgmgr_ctx.builtincursor, "/",
			      cfgmgr_materialize_cb, NULL);
	cfgmgr_dbgetnext(cfgmgr_ctx.usercursor, "/",
			      cfgmgr_materialize_cb, NULL);
}

static void
cfgmgr_detachrefs(bool commitsuccess) {
	ISC_LIST_FOREACH(cfgmgr_refstodetach, ref, link) {
		REQUIRE(ref->ptr);
		...
		isc_mem_put(cfgmgr_mctx, ref, sizeof(*ref));
	}
}
...
dns_cfgmgr_lmdb_commit(void) {
	 ...
	 * mode will be usable.
	 */
	if (cfgmgr_ctx.mode == DNS_CFGMGR_MODEUSER) {
		cfgmgr_materialize();
	}

	cfgmgr_closelmdbcursors();
	if (mdb_txn_commit(cfgmgr_ctx.txn) != 0) {
		cfgmgr_setlasterror("configuration database error");
		result = ISC_R_FAILURE;
	}
	cfgmgr_ctx.txn = NULL;

	if (cfgmgr_ctx.mode == DNS_CFGMGR_MODEBUILTIN) {
		cfgmgr_builtininitialized = true;
	} else if (cfgmgr_ctx.mode == DNS_CFGMGR_MODEUSER) {
		cfgmgr_userinitialized = true;
	}

	cfgmgr_requirenotxn();
	cfgmgr_detachrefs(result == ISC_R_SUCCESS);
	return result;
}
...
dns_cfgmgr_lmdb_rollback(void) {
	...
	cfgmgr_rollbacklmdbtxn();
	cfgmgr_requirenotxn();
	cfgmgr_detachrefs(false);
}

static size_t
cfgmgr_valuesz(const dns_cfgmgr_val_t *value) {
	size_t sz = sizeof(value->type);

	sz += sizeof(dns_cfgmgr_mode_t);
	switch (value->type) {
...
static void
cfgmgr_requirevalidpath(const char *path, bool hastrailing,
			     bool hasnottrailing) {
	REQUIRE(path != NULL);
	REQUIRE(path[0] == '/');
...
cfgmgr_inherit(const char *path, dns_cfgmgr_val_t *value) {
	...
	*(stpncpy(viewname, p1, p2 - p1)) = 0;
	...
	p2 = strchr(p1 + 1, '/');
	INSIST(*p2 != 0);
	cfgmgr_snprintf(newpath, sizeof(newpath), "/views/%s/%s", viewname,
			     p2 + 1);

read:
	result = dns_cfgmgr_read(newpath, value);
out:
	return result;
}
...
dns_cfgmgr_lmdb_read(const char *path, dns_cfgmgr_val_t *value) {
	...
	cfgmgr_requiretxn();
	cfgmgr_requirevalidpath(path, false, true);
	REQUIRE(value != NULL);

	result = cfgmgr_dbget(cfgmgr_dbcursor(), path, &dbval,
				   &dbvalsz);
	if (result != ISC_R_SUCCESS) {
		result = cfgmgr_inherit(path, value);
		goto out;
	}
...
dns_cfgmgr_lmdb_write(const char *path, const dns_cfgmgr_val_t *value) {
	...
	cfgmgr_requirerwtxn();
	cfgmgr_requirevalidpath(path, false, false);

	dbcursor = cfgmgr_dbcursor();

	result = cfgmgr_dbget(dbcursor, path, (void **)&dbval, &dbvalsz);
	if (result == ISC_R_SUCCESS && *dbval == DNS_CFGMGR_REF) {
		/*
		 ...
		 */
		if (detach) {
			ref = isc_mem_get(cfgmgr_mctx, sizeof(*ref));
			*ref = (detachref_t){ .ptr = ptr,
					       .detachonsuccess = detach,
					       .link = ISC_LINK_INITIALIZER };
			ISC_LIST_APPEND(cfgmgr_refstodetach, ref, link);
		}
	}
	...
	{
		if (result == ISC_R_SUCCESS) {
			cfgmgr_dbdel(dbcursor);
		}
		return;
	}

	REQUIRE(value->type != DNS_CFGMGR_UNDEFINED);
	if (value->type == DNS_CFGMGR_REF) {
		 ...
		 */
		dbcursor = cfgmgr_ctx.runningcursor;
		...
		if (value->detach) {
			detachref_t *ref = isc_mem_get(cfgmgr_mctx,
						       sizeof(*ref));
	...
	bdatalen = cfgmgr_valuesz(value);
	bdata = isc_mem_get(cfgmgr_mctx, bdatalen);
	...
	result = cfgmgr_dbput(dbcursor, path, bdata, bdatalen);
	isc_mem_put(cfgmgr_mctx, bdata, bdatalen);
}

static bool
cfgmgr_delete_cb(void *state, const char *nextkey, size_t nextkeylen,
		      const void *nextval, size_t nextvallen) {
	UNUSED(state);
	UNUSED(nextkey);
	UNUSED(nextkeylen);
...
dns_cfgmgr_lmdb_delete(const char *path) {
	cfgmgr_requirerwtxn();
	cfgmgr_requirevalidpath(path, true, false);

	INSIST(cfgmgr_dbgetnext(cfgmgr_dbcursor(), path, cfgmgr_delete_cb,
				NULL) == ISC_R_SUCCESS);
}
...
static bool
cfgmgr_foreach_cb(void *state, const char *nextkey, size_t nextkeylen,
		       const void *nextval, size_t nextvallen) {
	cfgmgr_foreach_ctx_t *ctx = state;
	size_t depth = 0;
	...
	/*
	 * See dns_cfgmgr_write: each cfgmgr values _always_ starts with
	...
	if ((*(unsigned char *)nextval) == DNS_CFGMGR_REF && skipref) {
		goto nextkey;
	}
	...
	if (ctx->property != NULL) {
		REQUIRE(dns_cfgmgr_read(nextkey, &value) == ISC_R_SUCCESS);
...
dns_cfgmgr_lmdb_foreach(const char *path, size_t maxdepth, void *state,
			...
			void (*labeldown)(void *state, const char *label),
			void (*labelup)(void *state)) {
	cfgmgr_foreach_ctx_t ctx = { .path = path,
					  .pathlen = strlen(path),
					  .maxdepth = maxdepth,
					  .state = state,
					  .property = property,
	...
	cfgmgr_dbgetnext(cfgmgr_dbcursor(), path,
			      cfgmgr_foreach_cb, &ctx);
...
static void
cfgmgr_refkey(isc_buffer_t *target, const void *owner, const char *path) {
	...
	for (size_t i = 0; i < sizeof(owner); i++) {
		isc_buffer_putuint8(target, hex[ownernum & 0xF]);
		ownernum >>= 4;
	}

	isc_buffer_putstr(target, path);
...
dns_cfgmgr_lmdb_setref(const void *owner, const char *path, void *ptr,
	...
	dns_cfgmgr_val_t val = { .type = DNS_CFGMGR_REF,
				 .ptr = ptr,
				 .attach = attach,
				 .detach = detach };

	isc_buffer_init(&b, bdata, sizeof(bdata));
	cfgmgr_refkey(&b, owner, path);
	dns_cfgmgr_lmdb_write(bdata, &val);
}
...
dns_cfgmgr_lmdb_getref(const void *owner, const char *path, void **ptr) {
	...
	isc_buffer_init(&b, bdata, sizeof(bdata));
	cfgmgr_refkey(&b, owner, path);
	result = cfgmgr_dbget(cfgmgr_dbcursor(), bdata, &dbval, &dbvalsz);
	if (result == ISC_R_SUCCESS) {
		isc_buffer_init(&b, dbval, dbvalsz);
		isc_buffer_add(&b, dbvalsz);

		INSIST(isc_buffer_getuint8(&b) == DNS_CFGMGR_REF);
		*ptr = (void *)isc_buffer_getuint48(&b);
	}
	return result;
}
