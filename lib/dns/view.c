...
static isc_result_t
bestzonecut_cache(dns_view_t *view, const dns_name_t *name, dns_name_t *fname,
		  dns_name_t *dcname, isc_stdtime_t now, unsigned int options,
...
bestzonecut_zoneorcache(dns_view_t *view, const dns_name_t *name,
	...
	result = bestzonecut_cache(view, name, cfname, cdcname, now, options,
				   delegsetp);
	if (result != ISC_R_SUCCESS) {
		return;
	}
...
dns_view_bestzonecut(dns_view_t *view, const dns_name_t *name,
	...
	if (result == DNS_R_NXDOMAIN && usecache) {
		 ...
		 * delegation.
		 */
		result = bestzonecut_cache(view, name, fname, dcname, now,
					   options, delegsetp);
