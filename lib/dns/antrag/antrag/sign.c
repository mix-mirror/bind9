...
static void
H(size_t m_len, const uint8_t m[], const uint8_t salt[], u16_poly *c1) {
	uint64_t state[25];
...
static int
check_norm(const u16_poly *p1, const u16_poly *p2) {
	int64_t s = 0;
	...
	for (int i = 0; i < ANTRAG_D / 2; ++i) {
		s += recenter(p1->coeffs[i]) *
		     recenter(p1->coeffs[i + ANTRAG_D / 2]);
		s += recenter(p2->coeffs[i]) *
		     recenter(p2->coeffs[i + ANTRAG_D / 2]);
	}
#endif
...
void
sampler(const secret_key *sk, const u16_poly *c2i, u16_poly *v1i,
	u16_poly *v2i) {
	poly c2, v1, v2;
	for (int i = 0; i < ANTRAG_D; i++) {
	...
	set_poly(&v1, x);
	set_poly(&v2, x);
	pointwise_mul(&v1, &(sk->b20));
	pointwise_mul(&v2, &(sk->b21));
	poly_sub(&temp_c1, &v1);
	poly_sub(&temp_c2, &v2); // Alg 13 line 14 c1 in the paper is
				 // [temp_c1,temp_c2] in the code
...
void
sign(size_t m_len, const uint8_t m[], const secret_key *sk, signature *s) {
...
int
verify(size_t m_len, const uint8_t m[], const public_key *pk,
       const signature *s) {
	u16_poly c1;
	H(m_len, m, s->r, &c1);
