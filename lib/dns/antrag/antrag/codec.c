...
encode_i8_vector(size_t off, uint8_t *buf, const int8_t *v) {
...
inline static size_t
encode_poly(size_t off, uint8_t *buf, const poly *p) {
	for (int i = 0; i < ANTRAG_D; i++) {
...
void
encode_sk(uint8_t *buf, const secret_key *sk) {
	size_t off = 0;
...
inline static size_t
decode_i8_vector(size_t off, const uint8_t *buf, int8_t *v) {
	assert(off + ANTRAG_D <= ANTRAG_SK_SIZE);
	for (int i = 0; i < ANTRAG_D; i++) {
		uint8_t b = buf[off++];
		v[i] = b < 0x80 ? b : b - 0x100; // b is promoted to int -> no
						 // overflow
	}
	return off;
}

static double __attribute__((__unused__))
decode_double(uint64_t y) {
	int s = y >> 63;
...
inline static size_t
decode_poly(size_t off, const uint8_t *buf, poly *p) {
	for (int i = 0; i < ANTRAG_D; i++) {
		uint64_t y = 0;
		assert(off + 8 <= ANTRAG_SK_SIZE);
		for (int j = 7; j >= 0; j--) {
...
void
decode_sk(secret_key *sk, const uint8_t *buf) {
	size_t off = 0;
...
static inline void
queue_bits(BitStream *bst, uint16_t n, int bit_cnt) {
...
dequeue_bits(BitStream *bst, int bit_cnt) {
...
static inline void
queue_byte(BitStream *bst, const uint8_t *buf, size_t *off) {
	queue_bits(bst, buf[(*off)++], 8);
}
static inline bool
has_queued_byte(BitStream *bst, bool pad) {
...
static inline void
dequeue_byte(BitStream *bst, uint8_t *buf, size_t *off) {
	buf[(*off)++] = dequeue_bits(bst, 8);
}

void
encode_pk(uint8_t *buf, const public_key *pk) {
...
void
decode_pk(public_key *pk, const uint8_t *buf) {
	BitStream bst = empty_stream;
	size_t off = 0;
	for (int i = 0; i < ANTRAG_D; i++) {
...
static int
encode_sig_vec(uint8_t *buf, const uint16_t s[ANTRAG_D]) {
	...
	for (int i = 0; i < ANTRAG_D; i++) {
		...
		km >>= SIG_SMALL_BITS;
		queue_bits(&bst, 1 << km,
			   km + 1); // extra bits in unary (at most
				    // 2^(LARGE_BITS-SMALL_BITS)+1)

		while (has_queued_byte(&bst, i == ANTRAG_D - 1)) {
			if (off >= SIG_VECTOR_SIZE)
				return 1;
			dequeue_byte(&bst, buf, &off);
		}
	}
...
int
encode_sig(uint8_t *buf, const signature *s) {
	if (encode_sig_vec(buf, s->s1.coeffs))
		return 1;
...
static int
decode_sig_vec(const uint8_t *buf, uint16_t s[ANTRAG_D]) {
	int large_val = 1 << SIG_LARGE_BITS;
...
int
decode_sig(signature *s, const uint8_t *buf) {
	if (decode_sig_vec(buf, s->s1.coeffs))
		return 1;
	for (int i = 0; i < SALT_BYTES; i++) {
