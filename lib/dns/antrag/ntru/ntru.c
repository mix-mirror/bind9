...
static bool
reduce_babai(const IPoly *f, const IPoly *g, IPoly *F, IPoly *G) {
	/*
	printf("reduce_babai start. sizes: |f|=%zu, |g|=%zu, |F|=%zu,
	|G|=%zu.\n", ipoly_size(f), ipoly_size(g), ipoly_size(F),
	ipoly_size(G));
	*/
	// compute constants
...
static bool
tower_solver(const IPoly *f, const IPoly *g, int d, int m, IPoly *F, IPoly *G) {
	if (d == 1) {
		mpz_ptr a = f->coefs[0];
		mpz_t b;
		mpz_init(b);
		...
		ipoly_new(&gn);
		/*
		printf("calling ipoly_mul from tower_solver (degs: %d, %d,
...
bool
solve_ntru(const int8_t fa[ANTRAG_D], const int8_t ga[ANTRAG_D],
	   int8_t Fa[ANTRAG_D], int8_t Ga[ANTRAG_D]) {
	IPoly f, g, F, G;
	...
	for (int i = 0; i < ANTRAG_D; i++) {
		assert(mpz_fits_slong_p(F.coefs[i]) &&
		       mpz_fits_slong_p(G.coefs[i]));
		long Fi = mpz_get_si(F.coefs[i]);
		long Gi = mpz_get_si(G.coefs[i]);
		if (Fi < INT8_MIN || Fi >= INT8_MAX || Gi < INT8_MIN ||
		    Gi >= INT8_MAX)
		{
