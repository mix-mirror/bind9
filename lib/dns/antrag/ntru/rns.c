...
mq_montymul(uint32_t x, uint32_t y, int qi) {
	uint64_t q = MODULI[qi].q;
	uint64_t z = (uint64_t)x * (uint64_t)y;
	z += ((z * MODULI[qi].m_inv_q) & 0xFFFFFFFF) * q; // z += z * (-q^-1) %
							  // 2^32 * q
...
static inline uint32_t
to_monty(uint32_t x, int qi) {
	return mq_montymul(x, MODULI[qi].r2, qi);
}

static inline uint32_t
from_monty(uint32_t x, int qi) {
	return mq_montymul(x, 1, qi);
}

static inline uint32_t
mq_add(uint32_t x, uint32_t y, int qi) {
	uint64_t q = MODULI[qi].q;
...
static inline uint32_t
mq_sub(uint32_t x, uint32_t y, int qi) {
...
void
rns_from_mpz(int mod_cnt, RNS(ri), const mpz_t mi) {
...
void
rns_to_mpz(int cnt, mpz_t mi[], int mod_cnt, const uint32_t ri[]) {
	mpz_t n1;
	...
	for (int qi = 1; qi < mod_cnt; qi++) {
		...
		for (int i = 0; i < cnt; i++) {
			...
			uint32_t a1mn2 = to_monty(mpz_fdiv_ui(mi[i], n2),
						  qi); // a1 mod n2 (montgomery)
			uint32_t x = mq_montymul(m1, mq_sub(a2, a1mn2, qi),
						 qi); // x = m1 (a2 - a1) mod n2
...
void
rns_cpy(int mod_cnt, RNS(dst), const RNS(src)) {
	for (int qi = 0; qi < mod_cnt; qi++) {
		dst[qi] = src[qi];
	}
}
void
rns_add(int mod_cnt, RNS(dst), const RNS(a), const RNS(b)) {
	for (int qi = 0; qi < mod_cnt; qi++) {
		dst[qi] = mq_add(a[qi], b[qi], qi);
	}
}
void
rns_sub(int mod_cnt, RNS(dst), const RNS(a), const RNS(b)) {
	for (int qi = 0; qi < mod_cnt; qi++) {
		dst[qi] = mq_sub(a[qi], b[qi], qi);
	}
}
void
rns_mul(int mod_cnt, RNS(dst), const RNS(a), const RNS(b)) {
	for (int qi = 0; qi < mod_cnt; qi++) {
...
static void
rns_root_pow(int mod_cnt, RNS(out), int pow, RNS(tmp)) {
	assert(pow >= 0);
	if (pow == 0) {
		for (int qi = 0; qi < mod_cnt; qi++) {
			out[qi] = 1;
...
void
rns_log(int mod_cnt, const RNS(a)) {
	for (int qi = 0; qi < mod_cnt; qi++) {
		printf(qi == 0 ? "{" : ",");
		printf("%u", a[qi]);
	}
...
bool
rns_eq(int mod_cnt, const RNS(a), const RNS(b)) {
	for (int qi = 0; qi < mod_cnt; qi++) {
		if (a[qi] != b[qi])
			return false;
	}
...
void
rnspoly_from_ipoly(RnsPoly *rp, const IPoly *ip, int mod_cnt) {
	rp->deg = ip->degree;
	rp->mod_cnt = mod_cnt;
...
void
rnspoly_to_ipoly(IPoly *ip, RnsPoly *rp) {
...
void
rnspoly_free(RnsPoly *rp) {
	free(rp->coefs);
}

void
rnspoly_log(const RnsPoly *rp) {
	for (int i = 0; i < rp->deg; i++) {
		printf(i == 0 ? "[" : ", ");
...
void
rns_ntt(RnsPoly *p, uint32_t tmp[RNS_BUF_SIZE]) {
...
void
rns_intt(RnsPoly *p, uint32_t tmp[RNS_BUF_SIZE]) {
	int mod_cnt = p->mod_cnt;
	int max_d = p->deg;
	int d = max_d;
	int s = 1;
			...
			if (d == 2 && ANTRAG_LOG3_D) {
				...
				for (int i = 0; i * 2 < d; i++) {
					...
					for (int o = 0; o < s; o++) {
						...
						rns_mul(mod_cnt, pa, pa,
							RNS_INV2);
						rns_mul(mod_cnt, pb, pb,
							RNS_INV2);
					}
				}
			}
