...
void
ipoly_zeros(IPoly *p, int degree) {
	p->degree = degree;
	p->capacity = degree;
...
void
ipoly_set_degree(IPoly *p, int degree) {
	if (degree > p->degree) {
...
void
ipoly_copy(IPoly *p2, const IPoly *p) {
...
void
ipoly_add(IPoly *f, const IPoly *g) {
...
void
ipoly_sub(IPoly *f, const IPoly *g) {
	assert(f->degree == g->degree);
	for (int i = 0; i < f->degree; i++) {
...
static void
ipoly_mul_naive(IPoly *h, const IPoly *f, const IPoly *g) {
	int n = f->degree;
	if (n == 1) {
...
static void
ipoly_mul_rns(IPoly *h, const IPoly *f, const IPoly *g) {
	RnsPoly f_rns, g_rns;
...
void
ipoly_mul(IPoly *h, const IPoly *f, const IPoly *g) {
	assert(f->degree == g->degree);
	int n = f->degree;
	if (n <= 64) {
...
void
ipoly_conj2(IPoly *pc, const IPoly *p) {
	ipoly_copy(pc, p);
...
void
ipoly_conj3(IPoly *pc, const IPoly *p) {
	// j = x^(d/2) - 1 → j^2 = -x^(d/2)
...
size_t
extra_bits(const IPoly *f, const IPoly *g, size_t max_bits) {
	size_t max = max_bits;
...
bool
ipoly_eq(const IPoly *f, const IPoly *g) {
	assert(f->degree == g->degree);
	for (int i = 0; i < f->degree; i++) {
