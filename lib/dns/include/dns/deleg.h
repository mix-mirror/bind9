 ...
 *
 * - an NS-based delegation, where associated server names and A/AAAA glues.o
 *
 ...
 * the DELEG ones. (This is resolver responsability to ensures that.)
 */
struct dns_delegset {
	unsigned int   magic;
	isc_mem_t     *mctx;
	...
	/*
	 * Used only when a delegation is build form a local zone.
	 */
	bool staticstub;
};
...
/*
 * Shutdown and detach the delegation database.i Allocated `dns_delegset_t`
 ...
 * (for convinience with ISC_LIST_FOREACH macros, _attach, _detach functions,
