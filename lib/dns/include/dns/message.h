...
#define MSG_SECTION_FOREACH(msg, section, elt)                                    \
        for (dns_linkedname_t *elt = ISC_LIST_HEAD((msg)->sections[(section)]); \
             elt != NULL;                                                             \
...
struct dns_message {
	...
	dns_linkedname_t *cursors[DNS_SECTION_MAX];
	dns_rdataset_t	 *opt;
	dns_rdataset_t	 *sig0;
	dns_rdataset_t	 *tsig;
	...
	dns_linkedname_t *tsigname; /* Owner name of TSIG, if any */
	...
	dns_linkedname_t *sig0name; /* Owner name of SIG0, if any */
	dst_key_t	 *sig0key;
...
dns_message_findname(dns_message_t *msg, dns_section_t section,
		     ...
		     dns_rdatatype_t covers, dns_linkedname_t **foundname,
...
void
dns_message_puttempname(dns_message_t *msg, dns_linkedname_t **item);
/*%<
