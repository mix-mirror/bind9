...
previous_closest_nsec(dns_rdatatype_t type, qpz_search_t *search,
	...
	for (;;) {
		if (*firstp) {
			 ...
			 */
			*firstp = false;
			result = dns_qp_lookup(&qpr, name, 0,
		...
		*nodep = NULL;
		result = dns_qp_lookup(
			&search->qpr, name, 0, DNS_DBNAMESPACE_NORMAL, NULL,
...
qpzone_find(dns_db_t *db, const dns_name_t *name, dns_dbversion_t *version,
	 ...
	 */
	result = dns_qp_lookup(&search.qpr, name, 0, nspace, NULL, &search.iter,
...
dbiterator_seek(dns_dbiterator_t *iterator,
	...
	case nsec3only:
		result = dns_qp_lookup(
			qpdbiter->snap, name, 0, DNS_DBNAMESPACE_NSEC3, NULL,
			&qpdbiter->iter, NULL, (void **)&qpdbiter->node, NULL);
		break;
	case nonsec3:
		result = dns_qp_lookup(
			qpdbiter->snap, name, 0, DNS_DBNAMESPACE_NORMAL, NULL,
			&qpdbiter->iter, NULL, (void **)&qpdbiter->node, NULL);
		break;
	case full:
		result = dns_qp_lookup(
