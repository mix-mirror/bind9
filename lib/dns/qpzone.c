...
find_wildcard(qpz_search_t *search, qpznode_t **nodep, const dns_name_t *qname,
	...
	for (int i = dns_qpchain_length(&search->chain) - 1; i >= 0; i--) {
		...
		if (wild) {
			...
			result = dns_qp_lookup_name(&search->qpr, wname, nspace,
						    &wit, NULL, (void **)&wnode,
						    NULL);
			if (result == ISC_R_SUCCESS) {
				/*
...
previous_closest_nsec(dns_rdatatype_t type, qpz_search_t *search,
	...
	for (;;) {
		...
		if (*firstp) {
			 ...
			 */
			*firstp = false;
			result = dns_qp_lookup_name(&search->qpr, name,
		...
		*nodep = NULL;
		result = dns_qp_lookup_name(
			&search->qpr, &nsec_node->name, DNS_DBNAMESPACE_NORMAL,
...
qpzone_find(dns_db_t *db, const dns_name_t *name, dns_dbversion_t *version,
	 ...
	 */
	result = dns_qp_lookup_name(&search.qpr, name, nspace, &search.iter,
...
dbiterator_first(dns_dbiterator_t *iterator DNS__DB_FLARG) {
	...
	case nsec3only:
		 ...
		 */
		result = dns_qp_lookup_name(
			qpdbiter->snap, &qpdb->common.origin,
...
dbiterator_last(dns_dbiterator_t *iterator DNS__DB_FLARG) {
	...
	case nonsec3:
		...
		result = dns_qp_lookup_name(
			qpdbiter->snap, &qpdb->common.origin,
			DNS_DBNAMESPACE_NSEC, &qpdbiter->iter, NULL,
			(void **)&qpdbiter->node, NULL);
		if (result == ISC_R_SUCCESS) {
...
dbiterator_seek3(dns_dbiterator_t *iterator,
	...
	result = dns_qp_lookup_name(qpdbiter->snap, name, DNS_DBNAMESPACE_NSEC3,
				    &qpdbiter->iter, NULL,
				    (void **)&qpdbiter->node, NULL);
...
qpzone_createiterator(dns_db_t *db, unsigned int options,
	...
	*iter = (qpdb_dbiterator_t){
	...
	case nsec3only:
		 ...
		 */
		result = dns_qp_lookup_name(iter->snap, &qpdb->common.origin,
