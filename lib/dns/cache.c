...
cache_create_db(dns_cache_t *cache, dns_db_t **dbp, isc_mem_t **tmctxp) {
	...
	dns_db_setcachesize(db, cache->size);
...
dns_cache_flushnode(dns_cache_t *cache, const dns_name_t *name, bool tree) {
	...
	/*
	 * The experimental LMDB cache does not maintain the qpcache node index
	 * used for targeted deletion.  Conservatively clear the whole LMDB cache
	 * so internal users such as NTA invalidation retain correct semantics
	 * without adding a second name index.
	 */
	LOCK(&cache->lock);
	if (cache->db != NULL) {
