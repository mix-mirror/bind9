...
struct qpcnode {
	 ...
	 * snapshot, while the two flags are written under the node lock and
	 * the tree write lock, and a write to a bit field would be a store
	 * to the whole byte that the lock-free readers share.
	 */
