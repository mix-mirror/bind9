...
find_coveringnsec(qpc_search_t *search, const dns_name_t *name,
	 ...
	 */
	result = dns_qp_lookup_name(search->qpdb->tree, name,
...
resume_iteration(qpc_dbit_t *qpdbiter, bool continuing) {
	...
	if (continuing && qpdbiter->node != NULL) {
		isc_result_t result;
		result = dns_qp_lookup_name(qpdb->tree, qpdbiter->name,
...
dbiterator_seek(dns_dbiterator_t *iterator,
	...
	result = dns_qp_lookup_name(qpdb->tree, name, DNS_DBNAMESPACE_NORMAL,
				    &qpdbiter->iter, NULL,
