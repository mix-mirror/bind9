 ...
 *
 *      Write transaction (the qp-trie's writer mutex)
 *
...
expire_header(qpcache_t *qpdb, qpcnode_t *node, dns_slabheader_t *header,
	...
	flush_node(qpdb, node, nlocktypep, qp,
 ...
 */
static void
delete_node(dns_qp_t *qp, qpcnode_t *node) {
	...
	result = dns_qp_deletekey(qp, node->key, node->keylen, NULL, NULL);
	if (result != ISC_R_SUCCESS) {
		isc_log_write(DNS_LOGCATEGORY_DATABASE, DNS_LOGMODULE_CACHE,
			      ISC_LOG_WARNING,
			      "delete_node(): "
...
/*
 * The caller must specify its current node lock status. It's okay for
 * no lock to be held if there are existing external references to the
 * node, but if this is the first external reference, then the caller
 * must be holding the node lock.
 *
...
qpcnode_release(qpcache_t *qpdb, qpcnode_t *node, isc_rwlocktype_t *nlocktypep,
	...
	if (qp != NULL) {
		/*
		 * We can delete the node if we are inside a write
		 * transaction.
		 */
		delete_node(qp, node);
	} else if (!node->deleted) {
		/*
		 * Otherwise we add this node to a linked list of nodes in
	...
	/* else: already out of the trie; its reference retires later */
unref:
	qpcnode_unref(node);
}
...
flush_node(qpcache_t *qpdb, qpcnode_t *node, isc_rwlocktype_t *nlocktypep,
	 ...
	 */
	qpcnode_acquire(qpdb, node, *nlocktypep DNS__DB_FLARG_PASS);
	qpcnode_release(qpdb, node, nlocktypep, qp DNS__DB_FLARG_PASS);
...
cleanup_deadnodes(qpcache_t *qpdb, uint16_t locknum) {
	...
	isc_queue_for_each_entry_safe(&deadnodes, qpnode, qpnext, deadlink) {
		qpcnode_release(qpdb, qpnode, &nlocktype, qp DNS__DB_FILELINE);
	}
...
qpcache_findnode(dns_db_t *db, const dns_name_t *name, bool create,
	...
	if (result == ISC_R_SUCCESS) {
		...
		result = reactivate_node(qpdb, node DNS__DB_FLARG_PASS);
	}
...
add(qpcache_t *qpdb, qpcnode_t *qpnode, dns_slabheader_t *newheader,
	...
	qpcache_miss(qpdb, newheader, &nlocktype, qp DNS__DB_FLARG_PASS);
