...
static size_t
expireheader(qpcache_t *qpdb, qpcnode_t *qpnode, dns_slabheader_t *header,
	     bool nodelocked, isc_rwlocktype_t *tlocktypep,
...
static void
expire_lru_headers(qpcache_t *qpdb, uint32_t locknum, size_t requested,
		   bool nodelocked,
	...
	do {
		dns_slabheader_t *header = ISC_SIEVE_NEXT(
			qpdb->buckets[locknum].sieve, visited, sievelink);
...
qpcache_miss(qpcache_t *qpdb, qpcnode_t *qpnode, dns_slabheader_t *newheader,
	     bool nodelocked, isc_rwlocktype_t *tlocktypep DNS__DB_FLARG) {
	...
	if (isc_mem_isovermem(qpdb->common.mctx)) {
		...
		expire_lru_headers(qpdb, locknum, purgesize, nodelocked,
				   tlocktypep DNS__DB_FLARG_PASS);
	}

	ISC_SIEVE_INSERT(qpdb->buckets[locknum].sieve, newheader, sievelink);
}
...
static void
delete_header_rcu(struct rcu_head *rcu_head) {
...
static void
delete_header(qpcache_t *qpdb, dns_slabheader_t **headerp) {
	dns_slabheader_t *header = MOVE_OWNERSHIP(*headerp);

	INSIST(!ISC_SIEVE_LINKED(header, sievelink));

	update_rrsetstats(qpdb->rrsetstats, header->typepair,
			  atomic_load_acquire(&header->attributes), false);

	/*
	 * This is safe as we first move the list to the .dirty list and then
	 * only clean this up when all references to node has been released.
	 * This means, there is no reader that could hold reference to this
	 * node while we are deleting it.
	 *
	 * Practically, we should be able to delete this directly, but ... lets
	 * follow the RCU semantics.
	 */
	...
	call_rcu(&header->rcu_head, delete_header_rcu);
}

static void
clean_cache_node(qpcache_t *qpdb, qpcnode_t *qpnode) {
	/*
	 * Mutual exclusion required.
	 */

	DNS_SLABHEADER_FOREACH_SAFE(header, &qpnode->dirty, dirtylink) {
		cds_list_del_rcu(&header->dirtylink);
		delete_header(qpdb, &header);
	}
}
...
delete_node(qpcache_t *qpdb, qpcnode_t *qpnode) {
	...
	case DNS_DBNAMESPACE_NORMAL:
		if (qpnode->havensec) {
			 ...
			 */
			result = dns_qp_deletename(qpdb->tree, &qpnode->name,
 ...
 */
static void
qpcnode_release(qpcache_t *qpdb, qpcnode_t *node, bool nodelocked,
		isc_rwlocktype_t *tlocktypep DNS__DB_FLARG) {
	...
	if (cds_list_empty_rcu(&node->dirty) &&
	    !cds_list_empty_rcu(&node->headers))
	{
		goto unref;
	}

	if (!cds_list_empty_rcu(&node->dirty)) {
		isc_mutex_t *nodelock = &qpdb->buckets[node->locknum].lock;
		if (!nodelocked) {
			LOCK(nodelock);
		}
	...
	if (!cds_list_empty_rcu(&node->headers)) {
		goto unref;
	}
...
mark_ancient(qpcache_t *qpdb, qpcnode_t *qpnode, dns_slabheader_t *header) {
	if (header == NULL || !mark(qpdb, header, DNS_SLABHEADERATTR_ANCIENT)) {
		return;
	}

	setttl(header, 0);
	ISC_SIEVE_UNLINK(qpdb->buckets[qpnode->locknum].sieve, header,
			 sievelink);
	cds_list_del_rcu(&header->headerlink);
	cds_list_add_rcu(&header->dirtylink, &qpnode->dirty);
}
...
expireheader(qpcache_t *qpdb, qpcnode_t *qpnode, dns_slabheader_t *header,
	...
	mark_ancient(qpdb, qpnode, header);
	mark_ancient(qpdb, qpnode, header->related);

	if (isc_refcount_current(&qpnode->erefs) == 0) {
		/*
		...
		qpcnode_release(qpdb, qpnode, nodelocked,
				tlocktypep DNS__DB_FLARG_PASS);
...
static void
fill_headers(dns_slabheader_t *source, dns_slabheader_t **headerp,
	     dns_slabheader_t **sigheaderp) {
	if (DNS_TYPEPAIR_TYPE(source->typepair) == dns_rdatatype_rrsig) {
		*sigheaderp = source;
		*headerp = source->related;
		return;
	}

	*headerp = source;
	*sigheaderp = source->related;
}
...
find_headers(qpcnode_t *node, qpc_search_t *search, dns_rdatatype_t type,
	...
	DNS_SLABHEADER_FOREACH_RCU(tmp, &node->headers, headerlink) {
...
qpcache_find(dns_db_t *db, const dns_name_t *name, dns_dbversion_t *version,
	...
	DNS_SLABHEADER_FOREACH_RCU(tmp, &node->headers, headerlink) {
		...
		/*
		 * We now know that there is at least one active non-stale
...
tree_exit:
	...
	if (search.need_cleanup) {
		...
		rcu_read_lock();
		qpcnode_release(search.qpdb, node, false,
...
qpcache_findrdataset(dns_db_t *db, dns_dbnode_t *node, dns_dbversion_t *version,
	...
	DNS_SLABHEADER_FOREACH_RCU(tmp, &qpnode->headers, headerlink) {
		dns_slabheader_t *header = NULL, *sigheader = NULL;
...
qpcnode_expiredata(dns_dbnode_t *node, void *data) {
	...
	/* mark_ancient() handles the related header automatically */
	(void)expireheader(qpdb, qpnode, header, false, &tlocktype,
...
cleanup_deadnodes(qpcache_t *qpdb, uint16_t locknum) {
	isc_rwlocktype_t tlocktype = isc_rwlocktype_none;
	isc_mutex_t *nodelock = &qpdb->buckets[locknum].lock;
	...
	isc_queue_for_each_entry_safe(&deadnodes, qpnode, qpnext, deadlink) {
		qpcnode_release(qpdb, qpnode, true,
...
new_qpcnode(qpcache_t *qpdb, const dns_name_t *name, dns_namespace_t nspace) {
	...
	*newdata = (qpcnode_t){
		...
		.dirty = CDS_LIST_HEAD_INIT(newdata->dirty),
	};
...
qpcnode_detachnode(dns_dbnode_t **nodep DNS__DB_FLARG) {
	...
	*nodep = NULL;
	...
	rcu_read_lock();
	qpcnode_release(qpdb, node, false, &tlocktype DNS__DB_FLARG_PASS);
	rcu_read_unlock();
...
add(qpcache_t *qpdb, qpcnode_t *qpnode, dns_slabheader_t *newheader,
	...
	DNS_SLABHEADER_FOREACH_RCU(tmp, &qpnode->headers, headerlink) {
		...
		if (EXISTS(newheader) && NEGATIVE(newheader)) {
			if (rdtype == dns_rdatatype_any) {
				 ...
				 */
				if (DNS_TYPEPAIR_TYPE(tmp->typepair) ==
				    dns_rdatatype_rrsig)
				{
					mark_ancient(qpdb, qpnode, header);
				}
		 ...
		 * check_ncache_block()
		 */
		if (ANCIENT(newheader)) {
			continue;
		}

		if (tmp->typepair == newheader->typepair) {
			INSIST(oldheader == NULL);
			oldheader = tmp;
		}

		if (rdtype == dns_rdatatype_rrsig &&
		    DNS_TYPEPAIR_TYPE(tmp->typepair) == covers)
		{
			INSIST(related == NULL);
			related = tmp;
		} else if (tmp->typepair == DNS_SIGTYPEPAIR(rdtype)) {
			INSIST(related == NULL);
			related = tmp;
		}
	}
	...
	if (oldheader != NULL) {
		...
		/* Transfer the related pointer from old to new header */
		if (oldheader->related != NULL) {
			newheader->related = MOVE_OWNERSHIP(oldheader->related);
			newheader->related->related = newheader;
		} else if (related != NULL) {
			newheader->related = related;
			newheader->related->related = newheader;
		}

		cds_list_add_rcu(&newheader->headerlink, &qpnode->headers);

		qpcache_miss(qpdb, qpnode, newheader, true,
			     &tlocktype DNS__DB_FLARG_PASS);

		mark_ancient(qpdb, qpnode, oldheader);
		/* oldheader->related moved to newheader->related */
		...
		if (overmaxtype(qpdb, ntypes)) {
			...
			if (NEGATIVE(newheader)) {
				/*
				 * Add the new negative header to the
				 * database only temporarily.
				 */
				expireheader = newheader;
			}

			mark_ancient(qpdb, qpnode, expireheader);
			mark_ancient(qpdb, qpnode, expireheader->related);
		}
	}
	...
	{
		mark_ancient(qpdb, qpnode, related);
	}
...
qpcache_addrdataset(dns_db_t *db, dns_dbnode_t *node, dns_dbversion_t *version,
	...
	if (rdataset->attributes.noqname) {
		RETERR(addnoqname(newheader->mctx, newheader, qpdb->maxrrperset,
				  rdataset));
	}
	if (rdataset->attributes.closest) {
		RETERR(addclosest(newheader->mctx, newheader, qpdb->maxrrperset,
				  rdataset));
	}

	nodelock = &qpdb->buckets[qpnode->locknum].lock;
...
rdatasetiter_first(dns_rdatasetiter_t *it DNS__DB_FLARG) {
	...
	DNS_SLABHEADER_FOREACH_RCU(tmp, &qpnode->headers, headerlink) {
		if (!EXISTS(tmp) || ANCIENT(tmp)) {
			if (!EXPIREDOK(iterator)) {
				continue;
			}
		}

		if (EXPIREDOK(iterator) || iterator_active(qpdb, iterator, tmp))
		{
...
rdatasetiter_next(dns_rdatasetiter_t *it DNS__DB_FLARG) {
	...
	if (from != NULL) {
		DNS_SLABHEADER_FOREACH_RCU_FROM(tmp, &qpnode->headers, from,
						headerlink) {
			if (!EXISTS(tmp) || ANCIENT(tmp)) {
				if (!EXPIREDOK(iterator)) {
					continue;
			...
			if (EXPIREDOK(iterator) ||
			    iterator_active(qpdb, iterator, tmp))
			{
				iterator->current = tmp;
				break;
			}
...
dereference_iter_node(qpc_dbit_t *qpdbiter DNS__DB_FLARG) {
	...
	qpcnode_release(qpdb, node, false,
...
qpcnode_destroy(qpcnode_t *qpnode) {
	...
	clean_cache_node(qpdb, qpnode);

	INSIST(cds_list_empty_rcu(&qpnode->dirty));

	DNS_SLABHEADER_FOREACH_SAFE(header, &qpnode->headers, headerlink) {
		ISC_SIEVE_UNLINK(qpdb->buckets[qpnode->locknum].sieve, header,
				 sievelink);
		cds_list_del_rcu(&header->headerlink);
		delete_header(qpdb, &header);
