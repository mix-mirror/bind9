...
dns_notify_destroy(dns_notify_t *notify, bool locked) {
	...
	if (notify->zone != NULL) {
		if (!locked) {
			dns_zone_lock(notify->zone);
		}
		REQUIRE(dns_zone_locked(notify->zone));
		nctx = dns_zone_getnotifyctx(notify->zone);
		if (ISC_LINK_LINKED(notify, link)) {
			ISC_LIST_UNLINK(nctx->notifies, notify, link);
		}
		if (!locked) {
			dns_zone_unlock(notify->zone);
		}
...
void
dns_notify_log(dns_notify_t *notify, int level, const char *fmt, ...) {
	va_list ap;
...
notify_done(void *arg) {
	...
	if (result == ISC_R_SUCCESS) {
		dns_notify_log(notify, ISC_LOG_DEBUG(3),
...
fail:
	...
	if (result == ISC_R_SUCCESS) {
		dns_notify_log(notify, ISC_LOG_DEBUG(1),
		...
		dns_notify_log(notify, ISC_LOG_NOTICE,
		...
		dns_notify_log(notify, ISC_LOG_WARNING,
			       "notify to %s failed: %s: retries exceeded",
			       addrbuf, isc_result_totext(result));
	} else {
		dns_notify_log(notify, ISC_LOG_WARNING,
...
notify_send_toaddr(void *arg) {
	...
	notifyctx = dns_zone_getnotifyctx(notify->zone);
	...
	if (!dns_zone_loaded(notify->zone) || notify->rlevent->canceled ||
	    dns_zone_exiting(notify->zone) || zmgr == NULL || view == NULL ||
	...
	if (notify->key != NULL) {
		...
		if (result != ISC_R_SUCCESS && result != ISC_R_NOTFOUND) {
			dns_notify_log(notify, ISC_LOG_ERROR,
	...
	if (key != NULL) {
		...
		dns_notify_log(notify, ISC_LOG_INFO,
			       "sending notify to %s : TSIG (%s)", addrbuf,
			       namebuf);
	} else {
		dns_notify_log(notify, ISC_LOG_INFO, "sending notify to %s",
			       addrbuf);
	}
	options = 0;
	if (view->peers != NULL) {
...
again:
	...
	dns_zonemgr_tlsctx_attach(zmgr, &zmgr_tlsctx_cache);
...
static isc_result_t
notify_queue(dns_notifyctx_t *nctx, dns_notify_t *notify, bool startup,
	     bool dequeue) {
	REQUIRE(DNS_NOTIFY_VALID(notify));

	isc_loop_t *loop = dns_zone_getloop(nctx->zone);
	dns_zonemgr_t *zmgr = dns_zone_getmgr(nctx->zone);
	isc_ratelimiter_t *notifyrl = NULL;
	isc_ratelimiter_t *startupnotifyrl = NULL;

	dns_zonemgr_getnotifyrl(zmgr, &notifyrl);
	dns_zonemgr_getstartupnotifyrl(zmgr, &startupnotifyrl);
...
dns_notify_dequeue(dns_notify_t *notify, bool startup) {
	return notify_queue(dns_zone_getnotifyctx(notify->zone), notify,
			    startup, true);
}
...
requeue:
	...
	{
		result = notify_queue(nctx, notify, true, true);
		if (result != ISC_R_SUCCESS) {
			return true;
		}

		notify->flags &= ~DNS_NOTIFY_STARTUP;
		result = notify_queue(nctx, notify, false, false);
		if (result != ISC_R_SUCCESS) {
			return false;
		}
	}
...
notify_isself(dns_notify_t *notify, isc_sockaddr_t *dst) {
	...
	notifyctx = dns_zone_getnotifyctx(notify->zone);
	view = dns_zone_getview(notify->zone);
	dns_zone_getisself(notify->zone, &isselffunc, &isselfarg);
...
notify_send(dns_notify_t *notify) {
	...
	REQUIRE(DNS_NOTIFY_VALID(notify));
	REQUIRE(dns_zone_locked(notify->zone));
	if (dns_zone_exiting(notify->zone)) {
		return;
	}
	notifyctx = dns_zone_getnotifyctx(notify->zone);
...
dns_notify_cancel(dns_notifyctx_t *nctx) {
	ISC_LIST_FOREACH(nctx->notifies, notify, link) {
		INSIST(dns_zone_locked(nctx->zone));
		if (notify->find != NULL) {
