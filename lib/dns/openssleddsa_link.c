...
openssleddsa_createctx(dst_key_t *key ISC_ATTR_UNUSED, dst_context_t *dctx) {
	...
	(void)openssleddsa_alg_info(dctx->key->key_alg);
...
openssleddsa_fromdns(dst_key_t *key, isc_buffer_t *data) {
	...
	len = r.length;
	result = raw_key_to_ossl(alginfo, 0, r.base, &len, &pkey);
...
openssleddsa_parse_priv(dst_key_t *key, dst_key_t *pub, dst_private_t *priv) {
	...
	if (label != NULL) {
		result = openssleddsa_fromlabel(key, label, NULL);
		if (result != ISC_R_SUCCESS) {
			return result;
		}
	...
	result = raw_key_to_ossl(alginfo, 1, priv->elements[privkey_index].data,
				 &len, &pkey);
	if (result != ISC_R_SUCCESS) {
		return result;
	}
...
openssleddsa_parse(dst_key_t *key, isc_lex_t *lexer, dst_key_t *pub) {
	...
	result = dst__privstruct_parse(key, DST_ALG_ED25519, lexer, mctx,
				       &priv);
	if (result != ISC_R_SUCCESS) {
		return result;
	}
...
openssleddsa_fromlabel(dst_key_t *key, const char *label, const char *pin) {
	...
	result = dst__openssl_fromlabel(alginfo->pkey_type, label, pin,
					&pubpkey, &privpkey);
	if (result != ISC_R_SUCCESS) {
		return result;
	}
