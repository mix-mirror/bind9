...
own size because chunks grow geometrically and are shrunk after an
update transaction, so a small chunk that is full of live twigs must
...
When a leaf is deleted, its value goes onto the transaction's dead
list; after the commit the list waits for a grace period, and for
any snapshot of an older version to be destroyed, before the
references are released. A value that is deleted and inserted again
before that has two references until the retirement drains.
