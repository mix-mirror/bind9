...
# A trailer naming a person by name and surname at an @isc.org address
# is a human co-author; there is no point in asking for confirmation.
HUMAN_COAUTHORED_BY_RE = re.compile(
    r"""
    ^ Co-Authored-By: \s+
      (?P<name>  [^<>@]+? \s+ [^<>@\s]+ )   # "Name Surname" (two words at least)
      \s+
      < (?P<email>  [^<>@\s]+ @ isc\.org ) >
    $
    """,
